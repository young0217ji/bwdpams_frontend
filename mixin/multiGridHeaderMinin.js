        ...
        .then(res => {
          ...
          const header = data
            ...
            .map((x, idx) => {
              ...
              if (
                x.PRIMARYKEYFLAG &&
                (x.CELLTYPE === 'EnumComboBox' || x.CELLTYPE === 'ComboBox')
              ) {
                cell = x.GRIDCOLUMNID;
                editable = x.PRIMARYKEYFLAG && x.EDITFLAG === 'Yes';
              }
              ...
              if (customDropDown) {
                for (let i = 0; i < headerParam.gridDropDownList.length; i++) {
                  if (
                    x.PRIMARYKEYFLAG &&
                    x.GRIDCOLUMNID === headerParam.gridDropDownList[i].dataVal
                  ) {
                    cell = x.GRIDCOLUMNID;
                    editable = x.PRIMARYKEYFLAG && x.EDITFLAG === 'Yes';
                  }
              ...
              if (!headerParam.xOverflowYn) {
                cellWidth = Math.floor(
                  ((divWrapper.offsetWidth - sysColWidth) / 100) *
          ...
          if (totalWidth < divWrapper.offsetWidth && !headerParam.xOverflowYn) {
            header[header.length - 1].width =
              header[header.length - 1].width +
              (divWrapper.offsetWidth - 7 - realWidth);
          }
        ...
        .then(res => {
          if (gridHeaderMergeItem != undefined) {
            gridHeaderMergeItem.forEach(x => {
              ...
              headerParam.gridHeader = headerParam.gridHeader.filter(
