      ...
      await this.mesGet({
        ...
        const header = data.filter(x => x.VISIBLEFLAG === "Yes").map((x) => {
          ...
          if(!this.headerParam.xOverflowYn){
            cellWidth = Math.floor(((this.$refs.divWrapper.offsetWidth - sysColWidth) / 100) * ((x.GRIDCOLUMNSIZE / totalWidth) * 100));
          }else{
        ...
        if(totalWidth < this.$refs.divWrapper.offsetWidth && !this.headerParam.xOverflowYn){
        ...
        if ( gridHeaderMergeItem != undefined) {
          gridHeaderMergeItem.forEach(x => {
            ...
            this.gridHeader = this.gridHeader.filter(y => x.field != y.HEADERMERGENAME);
