<template>
  <div>
    <ol class="page-navigation">
      ...
      <li>{{$t('MES_CommLang.MES_CommLang_00525')}}</li> <!-- 사용자 관리 -->
    </ol>
              ...
              <v-col :sm="9" :lg="10">
                <div class="form-group-wrap">
                  <div class="form-group">
                    <label>{{$t('MES_CommLang.MES_CommLang_00132')}}</label><!-- 사용자ID -->
                    <InputText
                      ref="userid"
                      :dataNm="'userid'"
                      :disabled="false"
          ...
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="3" align="left">
                <CardTitle>{{$t('UserRoleMngPage.label3')}}</CardTitle> <!-- 사용자 목록 -->
              </v-col>
      ...
      <v-col :cols="8"  :class="'divList1'" :style="{ height: '100%' }">
        <Card :style="{height : '100%'}">
          <cardBody :style="{width:'100%'}">
            <CardTitle>{{$t('MES_CommLang.MES_CommLang_00137')}}</CardTitle> <!-- 사용자상세 -->
                <div>
                    <v-row>
                      <v-col :cols="4">
                        <v-row class="search-box" align="center" no-gutters>
                          <v-col :cols="6">
                            <Label>
                              ...
                              {{$t('MES_CommLang.MES_CommLang_00132')}} <!-- 사용자ID -->
                            </Label>
                          </v-col>
                          <v-col :cols="6">
                            <InputText
                      ...
                      <v-col :cols="2">
                        ...
                        :disabled="idCheckDis"
                        :theme-color="'primary'" @click="userIdChk">{{$t('MES_CommLang.MES_CommLang_00438')}}</kbutton> <!-- 중복확인 -->
                      </v-col>
                      <v-col :cols="6">
                        <v-row class="search-box" align="center" no-gutters>
                          <v-col :cols="4">
                            <Label>
                              <v-icon x-small :color="'#fb8200'" class="mr-1">mdi-record-circle</v-icon>
                              {{$t('MES_CommLang.MES_CommLang_00099')}} <!-- 비밀번호 -->
                            </Label>
                          </v-col>
                          <v-col :cols="8">
                            <InputText
                      ...
                      <v-col :cols="6">
                        <v-row class="search-box" align="center" no-gutters>
                          <v-col :cols="4">
                            <Label>
                              <v-icon x-small :color="'#fb8200'" class="mr-1">mdi-record-circle</v-icon>
                              {{$t('MES_CommLang.MES_CommLang_00134')}} <!-- 사용자명 -->
                            </Label>
                          </v-col>
                          <v-col :cols="8">
                            <InputText
                      ...
                      <v-col :cols="4">
                        <v-row class="search-box" align="center" no-gutters>
                          <v-col :cols="6">
                            <Label>
                              <v-icon x-small :color="'#fb8200'" class="mr-1">mdi-record-circle</v-icon>
                              {{$t('MES_CommLang.MES_CommLang_00076')}} <!-- 권한 -->
                            </Label>
                          </v-col>
                          <v-col :cols="6">
                            <InputText
                      ...
                      <v-col :cols="2">
                        <kbutton
                         ref="roleBtn"
                        :theme-color="'primary'"
                        @click="[searchModalVisible = true, getUserInfo()]">{{$t('MES_CommLang.MES_CommLang_00522')}}</kbutton> <!-- 등록 -->
                      </v-col>
                    </v-row>
                    <v-row>
                      <v-col :cols="6">
                        <v-row class="search-box" align="center" no-gutters>
                          <v-col :cols="4">
                            <Label>
                              ...
                              {{$t('MES_CommLang.MES_CommLang_00437')}} <!-- 이메일 -->
                            </Label>
                          </v-col>
                          <v-col :cols="8">
                            <InputText
                      ...
                      <v-col :cols="6">
                        <v-row class="search-box" align="center" no-gutters>
                          <v-col :cols="4">
                            <Label>
                              <v-icon x-small :color="'primary'" class="mr-1">mdi-record-circle</v-icon>
                              {{$t('MES_CommLang.MES_CommLang_00439')}} <!-- 전화번호 -->
                            </Label>
                          </v-col>
                          <v-col :cols="8">
                            <InputText
                      ...
                      <v-col :cols="6">
                        <v-row class="search-box" align="center" no-gutters>
                          <v-col :cols="4">
                            <Label>
                              <v-icon x-small :color="'primary'" class="mr-1">mdi-record-circle</v-icon>
                              {{$t('MES_CommLang.MES_CommLang_00469')}} <!-- 등록자 -->
                            </Label>
                          </v-col>
                          <v-col :cols="8">
                            <InputText
...
export default {
  ...
  methods: {
    ...
    async saveBtn(){
      ...
      let passwordMsg = this.$i18n.t('MES_MsgLang.MES_MsgLang_00210'); /* 비밀번호를 입력해주세요. */
      let saveMsg1 = this.$i18n.t('MES_MsgLang.MES_MsgLang_00140'); /* 저장되었습니다. */
      let saveMsg2 = this.$i18n.t('MES_MsgLang.MES_MsgLang_00143'); /* 저장할 데이터가 없습니다. */
      ...
      for(let i = 0; i < saveData.length; i++){
        if( saveData[i].rowStat == "C" && saveData[i].NEWPASSWORD == "" ){
          this.$refs.alertPop.show(alertMsg, passwordMsg); /* 알림, 비밀번호를 입력해주세요. */
          return;
        }
      }
      ...
      if (saveData.length > 0) {
        const res = this.mesPost({
          ...
          messagename: 'TxnSFUser',
          sendParam: saveData,
        }).then(result => {
          this.$nextTick(() => {
            this.getGridData();
            if (result.returncode !== undefined && result.returncode === '0') {
              this.$refs.alertPop.show(alertMsg, saveMsg1); /* 알림, 저장되었습니다. */
            } else {
        ...
        this.$refs.alertPop.show(alertMsg, saveMsg2); /* 알림, 저장할 데이터가 없습니다. */
        return;
      }
    ...
    userIdChk(){
      ...
      if(count > 1){
        ...
        this.$refs['alertPop'].message = this.$i18n.t('MES_MsgLang.MES_MsgLang_00211');
        this.$refs['alertPop'].modalWidth = "300px";
        this.$refs['alertPop'].visibleDialog = true;
      }else{
        this.$refs['alertPop'].title = this.$i18n.t('MES_CommLang.MES_CommLang_00456');
        this.$refs['alertPop'].message = this.$i18n.t('MES_MsgLang.MES_MsgLang_00212');
        this.$refs['alertPop'].modalWidth = "300px";
        this.$refs['alertPop'].visibleDialog = true;
      }
    ...
    onRowClick(event) {
      ...
      event.dataItem[this.selectedField] = true;
        ...
        this.$refs['userId'].textVal = event.dataItem.USERID  ? event.dataItem.USERID: "";
        this.$refs['userName'].textVal = event.dataItem.USERNAME ? event.dataItem.USERNAME: "";
        this.$refs['email'].textVal = event.dataItem.EMAIL ? event.dataItem.EMAIL: "";
        this.$refs['phoneNumber'].textVal = event.dataItem.PHONE_NUMBER ? event.dataItem.PHONE_NUMBER: "";
        this.$refs['password'].textVal = event.dataItem.NEWPASSWORD ? event.dataItem.NEWPASSWORD: "";
        this.$refs['createUserID'].textVal = event.dataItem.CREATEUSERID ? event.dataItem.CREATEUSERID: "";
        this.$refs['createTime'].textVal = event.dataItem.CREATETIME ? event.dataItem.CREATETIME: "";
        this.$refs['lastUpdateUserID'].textVal = event.dataItem.LASTUPDATEUSERID ? event.dataItem.LASTUPDATEUSERID: "";
        this.$refs['lastUpdateTime'].textVal = event.dataItem.LASTUPDATETIME ? event.dataItem.LASTUPDATETIME: "";
        this.$refs['roleList'].textVal = event.dataItem.ROLENAME ? event.dataItem.ROLENAME: "";
        this.$refs['hideRoleID'].textVal = event.dataItem.ROLEID ? event.dataItem.ROLEID: "";
