<template>
    <div>
        <ol class="page-navigation">
            ...
            <li>{{ $t("MES_CommLang.MES_CommLang_00519") }}</li> <!-- 메뉴관리 -->
        </ol>
                            ...
                            <v-col :sm="9" :lg="10">
                                <div class="form-group-wrap">
                                    <div class="form-group">
                                        <label>{{ $t("MES_CommLang.MES_CommLang_00106") }}</label> <!-- 메뉴ID -->
                                        <InputText ref="s_menuID" :dataNm="'s_menuID'" :disabled="false"
                                            :searchOption="true" :searchFunc="searchBtn" @input-text-set="searchInput" />
                                    </div>
                                    <div class="form-group">
                                        <label>{{ $t("MES_CommLang.MES_CommLang_00107") }}</label> <!-- 메뉴명 -->
                                        <InputText ref="s_menuName" :dataNm="'s_menuName'" :disabled="false"
                                            :searchOption="true" :searchFunc="searchBtn" @input-text-set="searchInput" />
                                    </div>
                                    <div class="form-group">
                                        <label>{{ $t("MES_CommLang.MES_CommLang_00524") }}</label> <!-- 메뉴구분 -->
                                        <MesSelectBox :enumID="'MenuSystemType'" :allYN="true" :dataNm="'MenuSystemType'"
        ...
        <v-row ref="contents">
            <v-col :sm="3" :lg="2" :class="'divListWapper'" :style="{ height: '100%' }">
                <Card ref="gridCard" :style="{ height: '100%' }">
                    <CardBody :style="{ height: '100%' }">
                        <div ref="divWrapper" :style="{ height: '100%' }">
                            <KendoTree :treeData="treeData" :textField="LOCALECOLUMN" :activeItem="treeActiveItem"
            ...
            <v-col :sm="9" :lg="10" :class="'divListWapper'" :style="{ height: '100%' }">
                <Card ref="gridCard" :style="{ height: '100%' }">
                    <CardBody :style="{ width: '100%', height: '100%' }">
                        ...
                        <div ref="divWrapper" style="height: calc(100% - 33px)">
                            <KendoGrid ref="rowGrid" :gridHeight="gridHeight" :gridItems="gridData" :sortable="true"
                                :columns="gridHeader" :resizable="true" :edit-field="'inEdit'" :isPaging="true"
                                :selected-field="selectedField" :gridDropDownList="headerParam.enumComboBox"
                                :checkHeaderArr="headerParam.checkBox" :gridPageData="gridPageData" 
                                @selectionchange="onSelectionchange" @pageChangeEvent="pageChangeEvent"
...
let tmpI18n = localStorage.getItem("i18nVal") != null
    ...
    methods: {
        ...
        saveValidation() {
            ...
            if (validation.length > 0) {
                this.openAlert(this.$i18n.t('MES_CommLang.MES_CommLang_00414'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00213'));
                return;
            }
            ...
            if (dupArr.length > 0) {
                this.openAlert(this.$i18n.t('MES_CommLang.MES_CommLang_00414'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00214'));
                return;
            }
            this.saveBtn();
        },
        ...
        addBtn() {
            ...
            const dataItem = {
                ...
                MENUNAME_EN: '',
                ICONCLASS: '',
                PARENTSID: this.parentMenuID == this.$auth.$state.user.plantId ? '' : this.parentMenuID,
                POSITION: '',
        ...
        delConfirm() {
            ...
            this.$refs.ConfirmPop.message = this.$i18n.t('MES_CommLang.MES_CommLang_00106') + "(" + chk.join() + ") : " + this.$i18n.t('MES_MsgLang.MES_MsgLang_00089'); /* 선택한 데이터를 삭제 하시겠습니까? */
