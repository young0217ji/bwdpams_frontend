        ...
        <Card ref="gridCard2" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%', height: '100%' }">
            ...
            <div class="detail-area">
              <div
                class="left-area"
                style="display: block; float: left; width: 100% !important"
              >
                <div class="area-form">
                  <div class="area-th">
                    <label>Durable ID</label>
              ...
              <div
                ...
                "
              >
                <span class="i-msg" style="display: block; padding: 10px 0"
                  >※ 더블클릭하여 사유코드를 선택하세요.</span
                >
                <div
                  ref="divWrapper3"
...
import StandardDurable from '@/components/frmWhouseStockMgmt/StandardDurable.vue';
  ...
  methods: {
    ...
    async getGridData() {
      await this.mesGet({
        ...
        let changeState = '';
        if (this.gridData[0].DURABLESTATE == 'Created') {
    ...
    saveBtn() {
      ...
      const saveData = this.gridData
        .filter(x => x.DURABLEID == this.intputDurableId)
        .map(x => {
          return {
            DESCRIPTION: this.textAreaData,
    ...
    onRowClick(event) {
      ...
      if (event.dataItem.DURABLESTATENAME != '폐기') {
        this.changeStatus = this.statusChangeCodeList.filter(
          x => x.RESOURCESTATE === event.dataItem.DURABLESTATE
        )[0].CHANGESTATE;
        this.intputAfterStatus = this.statusChangeCodeList.filter(
          x => x.RESOURCESTATE === event.dataItem.DURABLESTATE
        )[0].CHANGESTATENAME;
        this.filterChangeCodeList = this.statusChangeCodeList.filter(
          x => x.RESOURCESTATE === event.dataItem.DURABLESTATE
        );
      ...
      if (
        this.intputNowStatus == '사용가능' ||
        this.intputNowStatus == '파손'
      ) {
        this.reasonDisable = false;
      } else if (
        this.intputNowStatus == '오염' ||
        this.intputNowStatus == '생성' ||
        this.intputNowStatus == '폐기' ||
        this.intputNowStatus == '사용중지'
      ) {
