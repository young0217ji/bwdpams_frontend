<template>
  <div>
    <ol class="page-navigation">
      ...
      <!-- 설비관리 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00633') }}</li>
      <!-- 긴급보전 이력조회 -->
    </ol>
              ...
              <v-col :sm="9" :lg="10">
                <div class="form-group-wrap">
                  ...
                  <div class="form-group" style="margin-top: 10px">
                    ...
                    <div class="form-group" style="margin-top: 10px">
                      <Label>{{ $t('MES_CommLang.MES_CommLang_00634') }}</Label>
                      <!-- 진행 상태 -->
                      <DropDownList
                        ref="cmbENUM"
                        :style="{ width: '100px' }"
              ...
              <CardBody :style="{ height: '100%' }" no-gutters>
                <v-row no-gutters>
                  <v-col cols="6" align="left">
                    <CardTitle>{{
                      $t('MES_CommLang.MES_CommLang_00635')
                    }}</CardTitle>
                    <!-- 긴급보전 목록 -->
                  </v-col>
                  <v-col cols="6" align="right">
                    <Button
                      ...
                      >{{ $t('MES_CommLang.MES_CommLang_00661') }}</Button
                    >
                    <!-- 소모자재조회 -->
                  </v-col>
                </v-row>
                <div ref="divWrapper1" style="height: calc(100% - 33px)">
                  <KendoGrid
                    ...
                    :gridItems="gridDataBMList"
                    :sortable="true"
                    :columns="BMDefinition.gridHeader"
                    :resizable="false"
                    :edit-field="'inEdit'"
                    :isPaging="true"
                    :selected-field="selectedField"
                    @gridrowclick="BMListRowClick"
                  />
                </div>
              </CardBody>
            </Card>
            ...
            <Card :style="{ height: '100%' }">
              <CardBody :style="{ height: '100%' }">
                <v-row no-gutters>
                  <v-col cols="6" align="left">
                    <CardTitle>{{
                      $t('MES_CommLang.MES_CommLang_00636')
                    }}</CardTitle>
                    <!-- 긴급보전 요청이력 -->
                  </v-col>
                </v-row>
                <div ref="divWrapper2" style="height: calc(100% - 33px)">
                  <KendoGrid
                    ref="BMDefinitionHistory"
                    :gridHeight="gridHeight"
                    :gridItems="BMHistory"
                    :sortable="true"
                    :columns="BMDefinitionHistory.gridHeader"
                    :resizable="false"
                    :edit-field="'inEdit'"
                    :isPaging="true"
                    :selected-field="selectedField"
    ...
    <EquipmentConsumable
      ref="EquipmentConsumableModal"
      v-if="EquipmentConsumableVisible"
      :visibleDialog="EquipmentConsumableVisible"
      @visibleDialog="val => (EquipmentConsumableVisible = val)"
      :title="EquipmentConsumableTitle"
      :BMCode="BMCode"
    />
  </div>
</template>
...
export default {
  ...
  async mounted() {
    ...
    this.EquipmentConsumableTitle = this.$i18n.t(
      'MES_CommLang.MES_CommLang_00215'
    ); //입고
    ...
    this.valueWorkcenter.text = this.$i18n.t(
  ...
  methods: {
    add() {
      ...
      this.EquipmentConsumableTitle =
        '소모자재조회 (' + this.selectedRequestId + ')'; //입고
