              ...
              <v-col cols="12" class="" align="left">
                <CardTitle
                  >설비상태 이력<span class="i-msg" v-if="click"
                    >{{ equipmentidtxt }}의 가동률 :
                    {{ availabilitytxt }}%</span
                  ></CardTitle
                >
              </v-col>
  ...
  methods: {
    ...
    async getGridData2() {
      ...
      if (this.todate === Utility.setFormatDate(todate, 'YYYYMMDD')) {
        ...
        this.sumTodate =
          Utility.setFormatDate(this.sumTodate, 'YYYY-MM-DD') + ' 23:59:59';
      }

      sumTime = this.calcDate(
        Utility.setFormatDate(this.sumTodate, 'YYYY-MM-DD HH:mm:ss'),
        Utility.setFormatDate(this.sumFromdate, 'YYYY-MM-DD HH:mm:ss')
      );
          ...
          if (this.runningList.length > 0) {
            ...
            for (let i = 0; i < equipmentFirstRunningDic.length; i++) {
              ...
              drSelectedArr = this.stateHistory
                .filter(
                  x =>
                    x.EQUIPMENTID === equipmentID && x.EQUIPMENTSTATE !== 'Run'
                )
                .map(x => {
                  return {
                    ...x,
              ...
              if (
                ...
                drSelectedArr.length === 0
              ) {
                const newRowUUid = Math.round(
                  Math.random() * 1000000000
                ).toString();
                runningTime =
                  runningTime +
                  this.calcDate(
                    Utility.setFormatDate(
                      this.sumTodate,
                      'YYYY-MM-DD HH:mm:ss'
                    ),
                    Utility.setFormatDate(
                ...
                this.gridData2Calc.push({
                  ...
                  COMMENT:
                    '(설비시작시간)' + equipmentFirstRunningDic[i].EVENTTIME,
                });
            ...
            for (let i = 0; i < this.stateHistory.length; ++i) {
              ...
              // 2-1. 같은 설비일 때
              if (
                equipmentID === this.stateHistory[newEquipmentIndex].EQUIPMENTID
              ) {
                if (equipmentState === 'Run' && !isEquipmentRunning) {
                  ...
                  if (i === this.stateHistory.length - 1) {
                    for (let j = 0; j < equipmentFirstRunningDic.length; j++) {
                      if (
                        equipmentID === equipmentFirstRunningDic[j].EQUIPMENTID
                      ) {
                    ...
                    this.gridData2Calc.push({
                      ...
                      RUNNINGTIME: this.calcTime(runningTime),
                      ACCRUNNINGTIME: this.calcTime(accRunningTime),
                      AVAILABILITY: ((accRunningTime / sumTime) * 100).toFixed(
                        2
                      ),
                    });
                  ...
                  runningTime =
                    ...
                    this.calcDate(
                      ...
                      Utility.setFormatDate(
                        this.stateHistory[newRunningIndex].EVENTTIME,
                        'YYYY-MM-DD HH:mm:ss'
                      )
                    );
                  accRunningTime = accRunningTime + runningTime;
                  this.gridData2Calc.push({
                    ...
                    EQUIPMENTID: equipmentID,
                    STARTSTATE:
                      this.stateHistory[newRunningIndex].EQUIPMENTSTATE,
                    STARTSTATENAME:
                      this.stateHistory[newRunningIndex].EQUIPMENTSTATENAME,
                ...
                if (isEquipmentRunning) {
                  if (this.stateHistory[i - 1].EQUIPMENTSTATE === 'Run') {
                    ...
                    this.gridData2Calc.push({
                      ...
                      idx: this.gridData2Calc.length,
                      EQUIPMENTID:
                        this.stateHistory[newRunningIndex].EQUIPMENTID,
                      STARTSTATE:
                        this.stateHistory[newRunningIndex].EQUIPMENTSTATE,
                      STARTSTATENAME:
                        this.stateHistory[newRunningIndex].EQUIPMENTSTATENAME,
                    ...
                    runningTime =
                      ...
                      this.calcDate(
                        Utility.setFormatDate(
                          this.stateHistory[i - 1].EVENTTIME,
                          'YYYY-MM-DD HH:mm:ss'
                        ),
                        Utility.setFormatDate(
                          this.stateHistory[newRunningIndex].EVENTTIME,
          ...
          gridData.map(x => {
            this.gridData2Calc.map(y => {
              if (x.EQUIPMENTID === y.EQUIPMENTID) {
                gridData[x.idx].AVAILABILITY =
                  this.gridData2Calc[y.idx].AVAILABILITY;
              }
            });
    ...
    toChange(e) {
      ...
      if (this.todate === Utility.setFormatDate(todate, 'YYYYMMDD')) {
        ...
        this.sumTodate =
          Utility.setFormatDate(e.value, 'YYYY-MM-DD') + ' 23:59:59';
      }
    },
    fromChange(e) {
      this.fromdate = Utility.setFormatDate(e.value, 'YYYYMMDD');
      this.sumFromdate =
        Utility.setFormatDate(e.value, 'YYYY-MM-DD') + ' 00:00:00';
    },
    ...
    calcTime(val) {
      ...
      let hour =
        parseInt(val / 3600) < 10
          ? '0' + parseInt(val / 3600)
          : parseInt(val / 3600);
      let min =
        parseInt((val % 3600) / 60) < 10
          ? '0' + parseInt((val % 3600) / 60)
