<template>
  <div>
    <ol class="page-navigation">
      ...
      <li>{{ $t("MES_CommLang.MES_CommLang_00637") }}</li> <!-- 긴급보전 요청조회 -->
    </ol>
        ...
        <Card ref="gridCard" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%', height: '100%' }">
            ...
            <div ref="divWrapper2" style="height: calc(100% - 33px);">
              <KendoGrid ref="BMReport" :gridHeight="gridHeight" :gridItems="gridDataBM" :sortable="true"
                :columns="BMListParam.gridHeader" :resizable="true" :isPaging="true" :selected-field="selectedField" />
            </div>
          </CardBody>
        </Card>
      </v-col>
...
export default {
  ...
  async mounted() {
    ...
    await this.getHeaderMulti(this.BMListParam, this.$refs.divWrapper2, this.customDropDown);
    this.getComboData();
  ...
  methods: {
    // 긴급보전 요청목록 그리드 데이터 가져오기
    async getBMGridData() {
      this.gridOriDataBM = [];
      this.gridDataBM = [];
      this.mesGet({
        apiKey: 'mes/common/getqueryresult',
        queryId: 'GetBMDefinition',
        queryVersion: '00001',
        sendParam: {
        ...
        this.gridOriDataBM = data;
        this.gridDataBM = data;
      });
    },
