<template>
  <div>
    <ol class="page-navigation">
      ...
      <!-- 설비관리 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00645') }}</li>
      <!-- 긴급보전 요청 -->
    </ol>
          ...
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="6" align="left">
                <CardTitle>{{
                  $t('MES_CommLang.MES_CommLang_00638')
                }}</CardTitle>
                <!-- 긴급보전 요청목록 -->
              </v-col>
              <v-col cols="6" align="right">
                <Button
                  ...
                  icon="save"
                  @click="requestBtn"
                  >{{ $t('MES_CommLang.MES_CommLang_00639') }}</Button
                >
                <!-- 요청 -->
                <Button
                  :theme-color="'primary'"
                  ...
                  icon="delete"
                  @click="cancelBtn"
                  >{{ $t('MES_CommLang.MES_CommLang_00410') }}</Button
                >
                <!-- 취소 -->
              </v-col>
            </v-row>
            <div ref="divWrapper2" style="height: calc(100% - 33px)">
              <KendoGrid
                ...
                :selected-field="selectedField"
                :dateTimePickerList="
                  EQMaintenanceHeaderParam.dateTimePickerList
                "
                @gridButtonClick="gridButtonClick"
                @selectionchange="onSelectionchange"
                @gridrowclick="requestListOnRowclick"
          ...
          <Card ref="gridCard" :style="{ height: '100%' }">
            <CardBody :style="{ width: '100%', height: '100%' }">
              <v-row no-gutters>
                <v-col cols="6" align="left">
                  <CardTitle>{{
                    $t('MES_CommLang.MES_CommLang_00640')
                  }}</CardTitle>
                  <!-- 요청 내용 -->
                </v-col>
              </v-row>
              <div class="detail-area">
                <div class="left-area">
                  <div class="area-form">
                    ...
                    <div class="area-th">
                      <label>{{ $t('MES_CommLang.MES_CommLang_00641') }}</label>
                      <!-- 요청자 -->
                    </div>
                    <div class="area-td">
                      <inputText
                  ...
                  <div class="area-form">
                    <div class="area-th">
                      <label>{{ $t('MES_CommLang.MES_CommLang_00642') }}</label>
                      <!-- 설비고장코드 -->
                    </div>
                    <div class="area-td">
                      <inputText
                    ...
                    <div class="area-th">
                      <label>{{ $t('MES_CommLang.MES_CommLang_00643') }}</label>
                      <!-- 설비고장코드명 -->
                    </div>
                    <div class="area-td">
                      <DropDownList
                        ref="cmbReasonCodeName"
                        :data-items="reasonCodeName"
                        :text-field="'text'"
                        :data-item-key="'id'"
                        :value="value3"
                        @change="comboChange3"
                  ...
                  <div class="area-form">
                    <div class="area-th">
                      <label>{{ $t('MES_CommLang.MES_CommLang_00644') }}</label>
                      <!-- 요청시간 -->
                    </div>
              ...
              <div class="detail-area" style="margin-top: 5px">
                <div class="area-form" style="height: 100px">
                  <div class="area-th">
                    <label>{{ $t('MES_CommLang.MES_CommLang_00640') }}</label>
                    <!-- 요청내용 -->
                  </div>
    ...
    <RequestCancelModal
      ref="RequestCancelModal"
      v-if="requestCancelModalVisible"
      :visibleDialog="requestCancelModalVisible"
      :title="requestCancelModalVisible"
      :selected-field="selectedField"
      :initData="initData"
      @visibleDialog="val => (requestCancelModalVisible = val)"
      @reuqestCancelData="reuqestCancelData"
    />
  </div>
</template>
<script>
...
export default {
  ...
  created() {
    ...
    this.setYYYYMM = Utility.setFormatDate(
      this.intputYearMonthDefault.replaceAll('-', ''),
      'YYYYMM'
    );
  ...
  methods: {
      ...
      this.mesGet({
        ...
        const data = res.map((x, idx) => {
          ...
          return {
            ...
            newRow: false,
            resizable: true,
            REQUESTTIME: Utility.setFormatDate(
              x.REQUESTTIME,
              'YYYY-MM-DD HH:mm:ss'
            ),
    ...
    async getReasonCodeComboData() {
      ...
      this.mesGet({
        ...
        for (let i = 0; i < data.length; i++) {
          if (i < data.length) {
            this.reasonCodeName.push({
              id: data[i].REASONCODE,
    ...
    // 요청
    requestBtn() {
      const requestData = this.gridData
        ...
        .map(x => {
          return {
            ...
            REQUESTTIME:
              this.intputRequestDateYYYYMMDD.replaceAll('-', '') +
              this.intputRequestDateHHMMSS.replaceAll(':', ''), // 요청일시
      ...
      if (this.reasoncode == '' || this.reasoncode == null) {
        this.$refs.alertPop.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00250')
        ); /* 설비고장코드를 선택해주세요. */
        return;
      }
    ...
    // 취소
    cancelBtn() {
      const delData = this.gridData2.filter(x => x.selected === true);
      ...
      if (delData.length > 0) {
        ...
        this.$refs.alertPop.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00252')
        ); /* 취소할 요청을 선택해주세요. */
      }
    },
    cancelModalVisible() {
    ...
    leftBtn() {
      ...
      if (parseInt(this.month) < 2) {
        ...
        this.setYYYYMM = Utility.setFormatDate(
          this.inputYearMonth.replaceAll('-', ''),
          'YYYYMM'
        );
    ...
    equipmentOnRowClick(event) {
      ...
      this.intputRequestDateYYYYMMDD = Utility.setFormatDate(
        new Date(),
        'YYYY-MM-DD'
      ); /* 요청시간 */
      this.intputRequestDateHHMMSS = Utility.setFormatDate(
        new Date(),
        'HH:mm:ss'
      ); /* 요청시간 */
