<template>
    <div>
      <ol class="page-navigation">
        ...
        <li>{{ $t("MES_CommLang.MES_CommLang_00645") }}</li> <!-- 긴급보전 요청 -->
      </ol>
      ...
      <v-row ref="contents">
        ...
        <v-col :sm="9" :lg="9" :class="'divListWapper'" :style="{ height: '55%' }">
          <Card ref="gridCard" :style="{ height: '100%' }">
            <CardBody :style="{ width: '100%', height: '100%' }">
              <v-row no-gutters>
                ...
                <v-col cols="6" align="right">
                  <Button :theme-color="'primary'" :size="'small'" icon="save" @click="requestBtn">{{ $t("MES_CommLang.MES_CommLang_00639") }}</Button> <!-- 요청 -->
                  <Button :theme-color="'primary'" :size="'small'" icon="delete" @click="cancelBtn">{{ $t("MES_CommLang.MES_CommLang_00410") }}</Button> <!-- 취소 -->
                </v-col>
              </v-row>
          ...
          <v-col :style="{ margin: '15px 0 0 0', height: '81%', width: '100%' }">
            <Card ref="gridCard" :style="{ height: '100%' }">
              <CardBody :style="{ width: '100%', height: '100%' }">
                <v-row no-gutters>
                  <v-col cols="6" align="left">
                    <CardTitle>{{ $t("MES_CommLang.MES_CommLang_00640") }}</CardTitle> <!-- 요청 내용 -->
                  </v-col>
                </v-row>
              ...
              <div class="left-area">
                <div class="area-form" >
                  ...
                  <div class="area-th">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00641") }}</label>  <!-- 요청자 -->
                  </div>
                  <div class="area-td">
                    <inputText
                      ref="intputRequestUser"
                      :boxWidth="'100%'"
                      :dataNm="'intputRequestUser'"
                      :disabled="false"
                      :readonly="true"
                      :defaultValue="intputRequestUser"
                      @input-text-set="searchInput"
                    />
                  </div>
                </div>
                <div class="area-form">
                <div class="area-th">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00642") }}</label> <!-- 설비고장코드 -->
                </div>
                <div class="area-td">
                  <inputText
                    ref="reasoncode"
                  ...
                  <div class="area-th">
                      <label>{{ $t("MES_CommLang.MES_CommLang_00643") }}</label> <!-- 설비고장코드명 -->
                    </div>
                    <div class="area-td">
                      <DropDownList ref="cmbReasonCodeName"  :data-items="reasonCodeName" :text-field="'text'"
                ...
                <div class="area-form">
                  <div class="area-th">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00644") }}</label> <!-- 요청시간 -->
                  </div>
            ...
            <div class="detail-area" style="margin-top: 5px;">
              <div class="area-form" style="height: 100px">
                <div class="area-th">
                  <label>{{ $t("MES_CommLang.MES_CommLang_00640") }}</label> <!-- 요청내용 -->
                </div>
      ...
      <RequestCancelModal ref="RequestCancelModal" v-if="requestCancelModalVisible" :visibleDialog="requestCancelModalVisible"
        :title="requestCancelModalVisible" :selected-field="selectedField"  :initData="initData"
        @visibleDialog="val => (requestCancelModalVisible = val)" @reuqestCancelData="reuqestCancelData" />
    </div>
  </template>
  <script>
  ...
  import RequestCancelModal from "@/components/frmEquipmentMaintenance/RequestCancelModal.vue";
  ...
  export default {
    ...
    data() {
      return {
        ...
        requestCancelModalVisible: false,
        initData: ''
      };
    },
    created() {
    ...
    async getReasonCodeComboData() {
      ...
      // 요청
      requestBtn() {
        const requestData = this.gridData.filter(x => x.EQUIPMENTID == this.intputEquipmentCode)
        .map(x => {
          return { 
            TARGETSTATUS: "Request",
            PLANTID: this.$auth.$state.user.plantId,
            EQUIPMENTID: this.intputEquipmentCode, /* 설비코드 */
            REASONCODE : this.reasoncode, // 설비고장코드
            REASONCODENAME: this.$refs['cmbReasonCodeName'].value.text,
            REQUESTUSERID: this.intputRequestUser, // 요청자
            REQUESTTIME: this.intputRequestDateYYYYMMDD.replaceAll('-', '') + this.intputRequestDateHHMMSS.replaceAll(':', ''), // 요청일시
            REQUESTDETAIL : this.requestCmmentTextArea, // 요청내용
          }
        });
        ...
        if(this.reasoncode == '' || this.reasoncode == null){
          this.$refs.alertPop.show(this.$i18n.t('MES_CommLang.MES_CommLang_00456'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00250')); /* 설비고장코드를 선택해주세요. */
          return;
        }
      ...
      cancelBtn() {
        ...
        if(delData.length > 0){
            ...
            this.$refs['confirmPop'].message = this.initData + ' - ' + this.$i18n.t('MES_MsgLang.MES_MsgLang_00251'); /* 해당 요청을 취소하시겠습니까? */
            this.$refs['confirmPop'].callBack = this.cancelModalVisible;
            this.$refs['confirmPop'].modalWidth = "300px";
            this.$refs['confirmPop'].visibleDialog = true;
            // this.requestCancelModalVisible = true;
        }else{
            this.$refs.alertPop.show(this.$i18n.t('MES_CommLang.MES_CommLang_00456'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00252')); /* 취소할 요청을 선택해주세요. */
        }
      },
      cancelModalVisible(){
        this.requestCancelModalVisible = true;
      ...
      requestListOnRowclick(event) {
        this.initData = event.dataItem.REQUESTID;
