<template>
                ...
                <CardBody :style="{ height: '100%' }" no-gutters>
                  ...
                  <div ref="divWrapper1" style="height: calc(100% - 33px);">
                    <KendoGrid ref="MaterialStatus" :gridHeight="gridHeight" :gridItems="gridDataMaterialStock"
                      :sortable="true" :columns="MaterialStatusHeaderParam.gridHeader" :resizable="false" :gridPageData="gridPageData"
                      :edit-field="'inEdit'" :isPaging="true" :selected-field="selectedField" @gridrowclick="MaterialStockRowClick" 
              ...
              <Card :style="{ height: '100%' }">
                <CardBody :style="{ height: '100%' }">
                  <v-row no-gutters>
                    ...
                    :theme-color="'secondary'"
                    :size="'small'"
                    @click="getExcelBottomGrid"
                    >전체EXCEL</kbutton
                    >
                    <kbutton
                    :theme-color="'secondary'"
                    :size="'small'"
                    @click="getExcelPagingBottomGrid"
                    >페이지EXCEL</kbutton
                    >
                    </v-col>
                  </v-row>
                  <div ref="divWrapper2" style="height: calc(100% - 33px);">
                    <KendoGrid ref="MaterialStockHistory" :gridHeight="gridHeight" :gridItems="gridDataMaterialStockHistory"
                      ...
                      @pageChangeEventBottom="pageChangeEventBottom"/>
                  </div>
                </CardBody>
              </Card>
            </v-col>
  ...
  export default {
    ...
    methods: {
      ...
      async pageChangeEventBottom(skip = 0, take = 20) {
            this.gridPageData = {
                skip: skip,
                take: take
            }
    ...
    getExcelBottomGrid(){
      if(this.gridDataMaterialStockHistory.length <= 0){
        this.$refs.alertPop.show(this.$i18n.t('MES_CommLang.MES_CommLang_00456'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00031')); /* 알림, 그리드 데이터가 없습니다. */
        return
      }
        this.excelGet({
            apiKey: 'mes/excel/customquery',
