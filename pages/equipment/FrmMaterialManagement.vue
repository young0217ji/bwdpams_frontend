<template>
  <div>
    <ol class="page-navigation">
      ...
      <li>{{ $t("MES_CommLang.MES_CommLang_00621") }}</li> <!-- 예비품 입/출고관리 -->
    </ol>
              ...
              <v-col :sm="9" :lg="10">
                <div class="form-group-wrap">
                  <div class="form-group" style="margin-left: -5px;">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00622") }}</label> <!-- 예비품 코드 -->
                    <InputText ref="sparePartId" :boxWidth="'85%'" :dataNm="'sparePartId'" :disabled="false"
                      :defaultValue="sparePartId" :searchOption="true" :searchFunc="searchBtn"
                      @input-text-set="searchInput" />
                  </div>
                  <div class="form-group" style="margin-left: -50px;">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00623") }}</label> <!-- 예비품명 -->
                    <InputText ref="sparePartName" :boxWidth="'85%'" :dataNm="'sparePartName'" :disabled="false"
                      :defaultValue="sparePartName" :searchOption="true" :searchFunc="searchBtn"
                      @input-text-set="searchInput" />
                  </div>
                </div>
              </v-col>
              ...
              <CardBody :style="{ height: '100%' }" no-gutters>
                <v-row no-gutters>
                  <v-col cols="6" align="left">
                    <CardTitle>{{ $t("MES_CommLang.MES_CommLang_00624") }}</CardTitle>
                    <!-- 재고현황 -->
                  </v-col>
                  <v-col cols="6" align="right">
                    <Button :theme-color="'primary'" :size="'small'" icon="add" @click="add">{{
                      $t("MES_CommLang.MES_CommLang_00215") }}</Button> <!-- 입고 -->
                    <Button ref="release" :theme-color="'secondary'" :size="'small'" :icon="'delete'" @click="release">
                      {{ $t("MES_CommLang.MES_CommLang_00625") }}</Button><!-- 출고 -->
                      <kbutton
                ...
                <div ref="divWrapper1" style="height: calc(100% - 33px);">
                  <KendoGrid ref="MaterialStatus" :gridHeight="gridHeight" :gridItems="gridDataMaterialStock"
                    :sortable="true"  :gridPageData="gridPageData" :columns="MaterialStatusHeaderParam.gridHeader" :resizable="false"
                    :edit-field="'inEdit'" :isPaging="true" :selected-field="selectedField" @pageChangeEvent="pageChangeEvent"
          ...
          <v-col :cols="12" :style="{ height: '100%' }">
            <Card :style="{ height: '100%' }">
              <CardBody :style="{ height: '100%' }">
                <v-row no-gutters>
                  <v-col cols="6" align="left">
                    <CardTitle>{{ $t("MES_CommLang.MES_CommLang_00626") }}</CardTitle> <!-- 입/출고이력 -->
                  </v-col>
                  <v-col cols="6" align="right">
                      <kbutton
                                :theme-color="'secondary'"
                ...
                <div ref="divWrapper2" style="height: calc(100% - 33px);">
                  <KendoGrid ref="MaterialStockHistory" :gridHeight="gridHeight" :gridItems="gridDataMaterialStockHistory"
                    :sortable="true" :columns="materialStockHistoryParam.gridHeader" :resizable="false" :gridPageData="gridPageData"
                    :edit-field="'inEdit'" :isPaging="true" :selected-field="selectedField" 
    ...
    <SparePartModal ref="SparePartModal" v-if="sparePartModalVisible" :visibleDialog="sparePartModalVisible"
      :gridData="sparePartGridData" :title="sparePartModalTitle" :selected-field="selectedField"
      @visibleDialog="val => (sparePartModalVisible = val)" @selectedRow="selectedRow" />

    <Warehousing ref="WarehousingModal" v-if="WarehousingModalVisible" :visibleDialog="WarehousingModalVisible"
      :title="WarehousingTitle" :workType="workType" @MaterialStockSaveData="MaterialStockSaveData"
      @visibleDialog="val => (WarehousingModalVisible = val)"
      :selectedMaterialId="selectedMaterialId" :selectedMaterialName="selectedMaterialName" :selectedWarehouseId="selectedWarehouseId"
...
import Warehousing from '@/components/frmMaterialManagement/Warehousing.vue';
...
export default {
  ...
  async mounted() {
    ...
    await this.getHeaderMulti(this.MaterialStatusHeaderParam, this.$refs.divWrapper1);
    await this.getHeaderMulti(this.materialStockHistoryParam, this.$refs.divWrapper2);
  ...
  methods: {
    MaterialStockRowClick(event) {
      ...
      this.getMaterialStockGridDataHistory(this.selectedWarehouseId,this.selectedMaterialId);
    },
