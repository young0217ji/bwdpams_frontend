<template>
  <div>
    <ol class="page-navigation">
      ...
      <li>{{ $t("MES_CommLang.MES_CommLang_00825") }}</li> <!-- 작업지시관리(도급업체) -->
    </ol>
              ...
              <v-col :sm="9" :lg="10">
                <div class="form-group-wrap">
                  <div class="form-group">
                    <Label>Line</Label>
                    <DropDownList ref="cmbLine" :boxWidth="'100%'" :data-items="line" :text-field="'text'"
                      :data-item-key="'id'" :value="value" @change="comboChange">
                    </DropDownList>
                  </div>
                  <div class="form-group" style="width: 272px; margin-left: 8px;">
                    <Label>WorkCenter</Label>
                    <DropDownList ref="cmbWorkCenter" :boxWidth="'80%'" :data-items="workCenter" :text-field="'text'"
                      :data-item-key="'id'" :value="value2" @change="comboChange2">
                    </DropDownList>
                  </div>
                  <div class="form-group">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00263") }}</label> <!-- 품번코드 -->
                    <InputText ref="productid" :boxWidth="'100%'" :dataNm="'productid'" :disabled="false"
                      :defaultValue="productid" :searchOption="true" :searchFunc="searchBtn"
                      @input-text-set="searchInput" />
                  </div>
                  <div class="form-group">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00260") }}</label> <!-- 품번명 -->
                    <InputText ref="productname" :boxWidth="'100%'" :dataNm="'productname'" :disabled="false"
                      :defaultValue="productname" :searchOption="true" :searchFunc="searchBtn"
                      @input-text-set="searchInput" />
                  </div>
                  <div class="form-group">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00704") }}</label> <!-- 입고 S/L -->
                    <InputText ref="warehousename" :boxWidth="'100%'" :dataNm="'warehousename'" :disabled="false"
                      :defaultValue="warehousename" :searchOption="true" :searchFunc="searchBtn"
                      @input-text-set="searchInput" />
                    <Button :theme-color="'primary'" @click="warehouseModalVisible = true">{{
                      $t("MES_CommLang.MES_CommLang_00169")
                    }}</Button> <!-- 선택 -->
                  </div>
                  <div class="form-group">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00532") }}</label> <!-- 작업지시번호 -->
                    <InputText ref="workorder" :boxWidth="'100%'" :dataNm="'workorder'" :disabled="false"
                      :defaultValue="workorder" :searchOption="true" :searchFunc="searchBtn"
                      @input-text-set="searchInput" />
                  </div>
                  <div class="form-group" style="width: 270px; margin-right: 0px;">
                    <Label>{{ $t("MES_CommLang.MES_CommLang_00052") }}</Label> <!-- 공정 -->
                    <DropDownList ref="cmbERPOper" :boxWidth="'120px'" :data-items="erpOper" :text-field="'text'"
                      :data-item-key="'id'" :value="value3" @change="comboChange3">
                    </DropDownList>
                  </div>
                  <div class="form-group">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00705") }}</label> <!-- 순서번호 -->
                    <InputText ref="sequenceNumber" :boxWidth="'100%'" :dataNm="'sequenceNumber'" :disabled="false"
                      :defaultValue="sequenceNumber" :searchOption="true" :searchFunc="searchBtn"
                      @input-text-set="searchInput" />
                  </div>

                  <div class="form-group" style="margin-right: 50px;">
                    <label>P/M</label>
                    <InputText ref="pm" :boxWidth="'100%'" :dataNm="'pm'" :disabled="false" :defaultValue="pm"
                      :searchOption="true" :searchFunc="searchBtn" @input-text-set="searchInput" />
                  </div>
                  <div class="form-group">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00706") }}</label> <!--계획일-->
                    <div class="form-datepicker">
                      <DatePicker ref="planFromDate" :default-value="new Date(planFromDate)" :dataNm="'planFromDate'"
                        :format="'yyyy-MM-dd'" @change="fromChange" />
                    </div>
                    <span class="i_tilde">~</span>
                    <div class="form-datepicker">
                      <DatePicker ref="plantToDate" :default-value="new Date()" :dataNm="'plantToDate'"
                  ...
                  <div class="form-group-wrap"
                    style="margin-top: 9px; border: 1.0px solid; border-radius: 5px; border-color: #d7dae4; height: 40px; width: 380px;">
                    <div class="form-group" style="margin-left: -50px; margin-bottom: 15px;">
                      <label>{{ $t("MES_CommLang.MES_CommLang_00410") }}</label> <!-- 취소 -->
                      <div class="form-chk">
                        <Checkbox ref="Cancel" :dataNm="'Cancel'" :disabled="false" :defaultchecked="false"
                          @checkbox-set="chkVal"></Checkbox>
                      </div>
                    </div>
                    <!-- </div> -->
                    <div class="form-group" style="margin-left: -50px; margin-bottom: 15px;">
                      <label>{{ $t("MES_CommLang.MES_CommLang_00707") }}</label> <!-- 대기 -->
                      <div class="form-chk">
                        <Checkbox ref="Created" :dataNm="'Created'" :disabled="false" :defaultchecked="true"
                          @checkbox-set="chkVal"></Checkbox>
                      </div>
                    </div>
                    <div class="form-group" style="margin-left: -50px; margin-bottom: 15px;">
                      <label>{{ $t("MES_CommLang.MES_CommLang_00708") }}</label> <!-- 진행 -->
                      <div class="form-chk">
                        <Checkbox ref="Released" :dataNm="'Released'" :disabled="false" :defaultchecked="true"
                          @checkbox-set="chkVal"></Checkbox>
                      </div>
                    </div>
                    <div class="form-group" style="margin-left: -50px; margin-bottom: 15px;">
                      ...
                      <div class="form-chk">
                        <Checkbox ref="Completed" :dataNm="'Completed'" :disabled="false" :defaultchecked="false"
                          @checkbox-set="chkVal"></Checkbox>
                      </div>
                    </div>
                    <div class="form-group" style="margin-left: -50px; margin-bottom: 15px;">
                      <label>{{ $t("MES_CommLang.MES_CommLang_00709") }}</label> <!-- 잔량 -->
                      <div class="form-chk">
                        <Checkbox ref="RemainQty" :dataNm="'RemainQty'" :disabled="false" :defaultchecked="false"
                          @checkbox-set="chkVal"></Checkbox>
                      </div>
                    </div>
                  </div>
          ...
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters style="margin-bottom: 5px;">
              ...
              <v-col cols="6" align="right">
                <Button :theme-color="'secondary'" :size="'small'" @click="statusBtn">Status</Button>
                <Button :theme-color="'secondary'" :size="'small'" @click="bomDeployBtn">{{
                  $t("MES_CommLang.MES_CommLang_00710") }}</Button> <!-- BOM전개 -->
                <Button :theme-color="'secondary'" :size="'small'" @click="stockBtn">{{
                  $t("MES_CommLang.MES_CommLang_00711") }}</Button> <!-- 재고정보 -->
                <Button :theme-color="'secondary'" :size="'small'" @click="showUbiform">Print</Button>
                <Button :theme-color="'secondary'" :size="'small'" @click="skLabelBtn">{{
                  $t("MES_CommLang.MES_CommLang_00712") }}</Button> <!-- S/K라벨 -->
                <Button :theme-color="'secondary'" :size="'small'" @click="getExcel">{{
                  $t("MES_CommLang.MES_CommLang_00676") }}</Button> <!-- 전체EXCEL -->
                <Button :theme-color="'secondary'" :size="'small'" @click="getExcelPaging">{{
                  $t("MES_CommLang.MES_CommLang_00677") }}</Button> <!-- 페이지EXCEL -->
                <Button :theme-color="'secondary'" :size="'small'" :icon="'undo'" @click="undoBtn">undo</Button>
              </v-col>
            </v-row>
            <div ref="divWrapper" style="height: calc(100% - 33px);">
              <KendoGrid ref="workorderGrid" :gridHeight="gridHeight" :gridItems="gridData" :sortable="true"
                :columns="headerParam.gridHeader" :resizable="true" :isPaging="true" :gridPageData="gridPageData"
                :selected-field="selectedField" :checkHeaderArr="checkHeaderArr" :reSetPage="this.reSetPage" @pageChangeEvent=pageChangeEvent @gridddchang="rowDdChange"
                @gridrowclick="onRowClick" />
            </div>
          </CardBody>
        </Card>
      </v-col>
      <v-col :sm="12" :lg="12" :class="'divListWapper'" :style="{ height: '53%' }">
        <Card ref="gridCard" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
            ...
            <div ref="divWrapper2" style="height: calc(100% - 33px);">
              <KendoGrid ref="workorderMaterialGrid" :gridHeight="gridHeight" :gridItems="gridData2" :sortable="true"
                :columns="headerParam2.gridHeader" 
                :resizable="true" :isPaging="false" :selected-field="selectedField" @gridddchang="rowDdChange2" />
            </div>
          </CardBody>
        </Card>
      </v-col>
    ...
    <WarehouseModal ref="WarehouseModal" v-if="warehouseModalVisible" :visibleDialog="warehouseModalVisible"
      :title="'title'" :message="'message'" :callBack="() => { }" @visibleDialog="(val) => warehouseModalVisible = val"
...
import WarehouseModal from "@/components/frmWorkorderMgmt/SelectWareHouseModal";
...
export default {
  ...
  data() {
    return {
      planFromDate: Utility.setFormatDate(new Date(year, month, day - 7), 'YYYY-MM-DD'),
  ...
  async mounted() {
    ...
    await this.getERPOperComboList();
  ...
  methods: {
    ...
    async getBMGridData() {
      ...
      if ( this.checkSubContractUser() == false ) {
        this.$refs.alertPop.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00278')
        ); /* 알림, 선택한 데이터가 없습니다.*/
        return;
      };
      ...
      await this.mesGet({
        ...
        const data = list.filter(x => this.RemainQty == 'RemainQty' ? ((x.PLANQUANTITY - x.GOODQTY - x.BADQTY) > 0 ? true : false) : true).map((x, idx) => {
          return {
            ...x,
    ...
    async getWorkCenterComboList(res) {
      ...
      await this.mesGet({
        apiKey: 'mes/common/getqueryresult',
        queryId: 'GetAreaListForSubContract',
        queryVersion: '00001',
        sendParam: {
          plantid: this.$auth.$state.user.plantId,
          AREATYPE: 'WorkCenter',
    ...
    async getERPOperComboList() {
      ...
      await this.mesGet({
        ...
        this.erpOper.push({ id: '', text: this.$i18n.t('MES_CommLang.MES_CommLang_00501') }); /* 전체 */
        this.value3 = this.erpOper[0];
        for (let i = 0; i < data.length; i++) {
          if (i < data.length) {
            this.erpOper.push({ id: data[i].PLANOPERID, text: data[i].PLANOPERNAME });
          }
        }
      });
    },
    ...
    async onRowClick(event) {
      ...
      await this.getWorkorderMaterial(event.dataItem);

    },
    async getWorkorderMaterial(data) {
      this.gridOriData2 = [];
      this.gridData2 = [];
      this.materialData = data;
      this.mesGet({
        apiKey: 'mes/common/getqueryresult',
        queryId: 'GetWorkOrderMaterial',
        queryVersion: '00001',
        sendParam: {
          PLANTID: this.$auth.$state.user.plantId,
    ...
    // 도급업체 사용자 설정 여부 체크 (WorkCenter 가 조회되지 않는 경우 도급업체 사용자가 아닌것으로 판단)
    checkSubContractUser() {
      if ( this.isNullOrEmpty(this.$refs['cmbWorkCenter'].value) || this.isNullOrEmpty(this.$refs['cmbWorkCenter'].value.id) ) {
        return false;
      }
      else {
        return true;
