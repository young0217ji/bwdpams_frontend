<template>
  <div>
    <ol class="page-navigation">
      ...
      <!-- 생산관리 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00689') }}</li>
      <!-- MES Lot 이력 조회 -->
    </ol>
              ...
              <v-col :sm="9" :lg="10" align="left">
                <div class="form-group-wrap">
                  <div class="form-group">
                    <label>{{ $t('MES_CommLang.MES_CommLang_00544') }}</label>
                    <!-- 품번그룹 -->
                    <DropDownList
                      ref="cmbProGroup"
                      :style="{ width: '250px' }"
                  ...
                  <div class="form-group">
                    <label>{{ $t('MES_CommLang.MES_CommLang_00258') }}</label
                    ><!--품번-->
                    <InputText
                      :dataNm="'productId'"
                  ...
                  <div class="form-group">
                    <label>{{ $t('MES_CommLang.MES_CommLang_00553') }}</label
                    ><!--품명-->
                    <InputText
                      :dataNm="'productName'"
                      @input-text-set="searchInput"
          ...
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="6" align="left">
                <CardTitle>{{
                  $t('MES_CommLang.MES_CommLang_00529')
                }}</CardTitle>
                <!-- W/O 정보 -->
              </v-col>
            </v-row>
        ...
        <Card ref="gridCard" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="6" align="left">
                <CardTitle>{{
                  $t('MES_CommLang.MES_CommLang_00691')
                }}</CardTitle>
                <!-- Lot 목록 -->
              </v-col>
            </v-row>
        ...
        <Card ref="gridCard" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="6" align="left">
                <CardTitle>{{
                  $t('MES_CommLang.MES_CommLang_00692')
                }}</CardTitle>
                <!-- Lot 이력 목록 -->
              </v-col>
            </v-row>
...
export default {
  ...
  data() {
    return {
      ...
      headerParam3: {
        gridid: 'dgvLotProcessHistory', // 그리드 ID
        gridHeader: [],
        rowStat: '',
        noRowstat: true,
        noFirstCheck: true,
  ...
  methods: {
        ...
        for (let i = 0; i < res.length; i++) {
          if (i < res.length) {
            this.productGroup.push({
              id: res[i].PRODUCTGROUPID,
              text: res[i].PRODUCTGROUPNAME,
            });
    ...
    async getGridData() {
      ...
      this.mesGet({
        apiKey: 'mes/common/getqueryresult',
        //조수완 : GetWorkOrderList에서 수정
        queryId: 'GetWrkOrdList',
        queryVersion: '00004',
        sendParam: {
          plantid: this.$auth.$state.user.plantId,
    ...
    async getGridData2(event) {
      ...
      this.mesGet({
        ...
        this.getGridData3(this.gridData2[0].LOTID);
      });
    },
    async getGridData3(event) {
      ...
      this.mesGet({
        apiKey: 'mes/common/getqueryresult',
        //조수완 GetLotProcessHistoryList에서 수정
        queryId: 'GetLotProcHstList',
        queryVersion: '00001',
        sendParam: {
          plantid: this.$auth.$state.user.plantId,
          LOTID: event,
    ...
    onRowClick(event) {
      ...
      this.getGridData3(event.dataItem.LOTID);
    },
    onRowClick2(event) {
