<template>
  <div>
    <ol class="page-navigation">
      ...
      <!-- 공정관리 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00825') }}</li>
      <!-- 작업지시관리(도급업체) -->
    </ol>
...
export default {
  ...
  methods: {
    ...
    async getBMGridData() {
      ...
      if (this.checkSubContractUser() == false) {
        this.$refs.alertPop.show(
          ...
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00278')
        ); /* 알림, 선택한 데이터가 없습니다.*/
        return;
      }
    ...
    async onRowClick(event) {
      ...
      await this.GetWrkOrdMat(event.dataItem);
    },
    async GetWrkOrdMat(data) {
      this.gridOriData2 = [];
      this.gridData2 = [];
    ...
    checkSubContractUser() {
      if (
        this.isNullOrEmpty(this.$refs['cmbWorkCenter'].value) ||
        this.isNullOrEmpty(this.$refs['cmbWorkCenter'].value.id)
      ) {
        return false;
      } else {
        return true;
