<template>
  <div>
    <ol class="page-navigation">
      ...
      <!-- 홈 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00065') }}</li>
      <!-- 공정진행 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00905') }}</li>
      <!-- 이상발생 조회 -->
    </ol>
              ...
              <v-col :sm="9" :lg="10">
                <div class="form-group-wrap">
                  ...
                  <div class="form-group">
                    ...
                    <!-- 상태 -->
                    <MesSelectBox
                      :enumID="'AbnormalState'"
                      :allYN="true"
                      :dataNm="'AbnormalState'"
                      @comboChange="comboChange"
                    ></MesSelectBox>
                  </div>
                </div>
            ...
            <v-row no-gutters>
              ...
              <v-col cols="6" align="right">
                <kbutton
                  ...
                  :size="'small'"
                  @click="arrivalBtn"
                  >{{ $t('MES_CommLang.MES_CommLang_00906') }}</kbutton
                >
                <!-- 도착등록 -->
                <kbutton
                  :theme-color="'primary'"
                  :size="'small'"
                  @click="finishBtn"
                  >{{ $t('MES_CommLang.MES_CommLang_00907') }}</kbutton
                >
                <!-- 조치등록 -->
                <kbutton
                  :theme-color="'primary'"
                  :size="'small'"
                  @click="additionalBtn"
                  >{{ $t('MES_CommLang.MES_CommLang_00959') }}</kbutton
                >
                <!-- 추가등록 -->
              </v-col>
            </v-row>
    ...
    <ArrivalRegister
      ref="arrivalRegister"
      v-if="arrivalRegisterVisible"
      :visibleDialog="arrivalRegisterVisible"
      :title="'title'"
      :message="'message'"
      :initData="initData"
      :callBack="() => {}"
      @visibleDialog="val => (arrivalRegisterVisible = val)"
      @arrivalSave="arrivalSave"
    />
    <FinishRegister
      ref="finishRegister"
      v-if="finishRegisterVisible"
      :visibleDialog="finishRegisterVisible"
      :title="'title'"
      :message="'message'"
      :initData="initData"
      :callBack="() => {}"
      @visibleDialog="val => (finishRegisterVisible = val)"
      @finishSave="finishSave"
    />
    <AdditionalRegister
      ref="additionalRegister"
      v-if="additionalRegisterVisible"
      :visibleDialog="additionalRegisterVisible"
      :title="'title'"
      :message="'message'"
      :initData="initData"
      :callBack="() => {}"
      @visibleDialog="val => (additionalRegisterVisible = val)"
      @additionalSave="additionalSave"
    />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async arrivalBtn() {
      if (
        this.selected.ABNORMALSTATE == '' ||
        this.selected.ABNORMALSTATE == null ||
        this.selected.ABNORMALSTATE == undefined
      ) {
        this.$refs.alertPop.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
      ...
      if (this.selected.ABNORMALSTATE == 'Occurred') {
        ...
        this.$refs.alertPop.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00289')
        ); /* 알림, 도착등록은 신고 상태 일 경우에만 가능합니다. */
      }
    },
    async finishBtn() {
      if (this.selected.ABNORMALSTATE == 'Precessing') {
        ...
        this.$refs.alertPop.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00290')
        ); /* 알림, 조치등록은 처리중 상태 일 경우에만 가능합니다. */
      }
    },
    async additionalBtn() {
      if (this.selected.ABNORMALSTATE == 'Completed') {
        ...
        this.$refs.alertPop.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00296')
        ); /* 알림, 조치등록은 처리완료 상태 일 경우에만 가능합니다. */
      }
    },
    arrivalSave(res) {
      if (res) {
