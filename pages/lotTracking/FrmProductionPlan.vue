      ...
      <v-col :cols="12" :style="{ height: '100%' }">
        <Card ref="gridCard" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%', height: '100%' }">
            ...
            <div
              ref="divWrapper"
              :style="{ 'margin-top': '10px', height: 'calc(100% - 43px)' }"
            >
              <KendoGrid
                ref="rowGrid"
  ...
  methods: {
      ...
      this.gridData = this.gridData.map(x => {
        if (x.rowStat === 'C') {
          if (x.newRowUUid === this.newRowUUid) {
            return {
              ...x,
              VENDOR: data.COMPANYID,
              VENDORNM: data.COMPANYNAME,
              [`N${x.newRowUUid}USERNAME`]: data.COMPANYID,
            };
          }
        } else {
          if (x.newRowUUid === this.newRowUUid) {
    ...
    saveValidation() {
      ...
      const validation2 = this.gridData.filter(
        x => x.rowStat !== 'N' && !x.PLANSTARTDATE
      );
      if (validation2.length > 0) {
        this.openAlert('저장', '계획시작일 값은 필수 입력값입니다.');
        return;
      }
      const validation3 = this.gridData.filter(
        x => x.rowStat !== 'N' && !x.PLANENDDATE
      );
      if (validation3.length > 0) {
        this.openAlert('저장', '계획종료일 값은 필수 입력값입니다.');
        return;
      }
      const validation4 = this.gridData.filter(
        x => x.rowStat !== 'N' && !x.PLANORDERQTY
      );
      ...
      saveData.forEach(x => {
        timeChk.push(
          moment(x.PLANSTARTDATE, 'YYYY-MM-DD', true).isValid().toString()
        );
      });
      ...
      saveData.forEach(x => {
        timeChk2.push(
          moment(x.PLANENDDATE, 'YYYY-MM-DD', true).isValid().toString()
        );
      });
      ...
      saveData.forEach(x => {
        if (x.DUEDATE) {
          timeChk3.push(
            moment(x.DUEDATE, 'YYYY-MM-DD', true).isValid().toString()
          );
        }
      });
