<template>
  <div>
    <ol class="page-navigation">
      ...
      <!-- 공정관리 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00855') }}</li>
      <!-- 작업 일보 현황 -->
    </ol>
          ...
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="6" align="left">
                <CardTitle>{{
                  $t('MES_CommLang.MES_CommLang_00855')
                }}</CardTitle>
                <!-- 작업 일보 현황 -->
              </v-col>
            </v-row>
    ...
    <SelectProcess
      ref="SelectProcess"
      v-if="SelectProcessVisible"
      :visibleDialog="SelectProcessVisible"
      :title="SelectProcessTitle"
      :selected-field="selectedField"
      :initData="initData"
      @visibleDialog="val => (SelectProcessVisible = val)"
      @selectedRow="selectedProcess"
    />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    fromChange(e) {
      let checkDay = DateUtility.checkDiffDay(
        e.value,
      ...
      if (checkDay) {
        this.$refs.alertPop.show(
          ...
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00284')
        ); /* 알림, 조회기간은 30일을 넘을 수 없습니다. */
    ...
    async getGridData() {
      if (
        this.$refs['processid'].textVal == '' ||
        this.$refs['processid'].textVal == null ||
        this.$refs['equipmentid'].textVal == '' ||
        this.$refs['equipmentid'].textVal == null
      ) {
        this.$refs.alertPop.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00283')
        ); /* 알림, 공정ID와 설비코드를 선택해주세요. */
        return;
      }

      if (
        0 > DateUtility.diff(new Date(this.startDate), new Date(this.endDate))
      ) {
        this.$refs.alertPop.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00285')
        ); /* 알림, 종료 일자가 시작 일자보다 이전 일 수 없습니다. */
        return;
      }
