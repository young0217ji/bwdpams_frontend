<template>
  <div>
    <ol class="page-navigation">
      ...
      <!-- 생산관리 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00530') }}</li>
      <!-- Lot 생성 -->
    </ol>
                ...
                <v-row no-gutters>
                  ...
                  <v-col cols="6" align="right">
                    <Button
                      ...
                      :size="'small'"
                      icon="add"
                      @click="lotCreateModalVisible = true"
                      :disabled="confirm"
                    >
  ...
  computed: {
    items() {
      return this.gridData.map(item => ({
        ...item,
        selected: item.PRODUCTORDERID === this.selectedID,
      }));
    },
  },
  watch: {},
  methods: {
          ...
          this.$nextTick(() => {
            if (result.returncode !== undefined && result.returncode === '0') {
              this.$refs.alertPop2.show(
                ...
                this.$i18n.t('MES_MsgLang.MES_MsgLang_00221')
              ); /* 알림, 생성취소가 완료되었습니다. */
              this.getGridData2();
            }
          });
    ...
    saveBtn(data) {
      if (data !== undefined && data === '0') {
        this.$refs.alertPop2.show(
          this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00222')
        ); /* 알림, 생성이 완료되었습니다. */
        this.getGridData2();
      }
    },
