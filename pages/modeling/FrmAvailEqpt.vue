              ...
              <v-col :sm="3" :lg="2" align="right">
                <div class="form-chk" style="margin-right: 30px">
                  <Checkbox
                    ref="chkAllProduct"
                    :value="true"
                    :dataNm="'chkAllProduct'"
                    :required="true"
                    :searchOption="true"
                    @checkbox-set="formCheckValSet"
                  ></Checkbox>
                  <label>{{ $t('MES_CommLang.MES_CommLang_00555') }}</label>
                  <!-- 전품목설정 -->
                </div>
                <kbutton
                  :theme-color="'primary'"
              ...
              <CardBody :style="{ width: '100%', height: '100%' }">
                ...
                <div ref="divWrapper" :style="{ height: 'calc(100% - 33px)' }">
                  <KendoGrid
                    ...
                    :isPaging="true"
                    @gridrowclick="productOnRowClick"
            ...
            <Card ref="gridCard" :style="{ height: '100%' }">
              <CardBody :style="{ width: '100%', height: '100%' }">
                <v-row no-gutters>
                  <v-col cols="6" align="left">
                    <CardTitle>{{
                      $t('MES_CommLang.MES_CommLang_00056')
                    }}</CardTitle>
                    <!-- 공정목록 -->
                  </v-col>
                </v-row>
                <div ref="divWrapper2" :style="{ height: 'calc(100% - 33px)' }">
                  <KendoGrid
                    ref="processGrid"
                    :gridHeight="gridHeight"
                    :gridItems="processItems"
                    :gridPageData="gridPageData2"
                    :sortable="true"
                    ...
                    :selected-field="selectedField"
                    :isPaging="true"
                    @gridrowclick="processOnRowClick"
                ...
                <Card :style="{ height: '100%' }">
                  <CardBody :style="{ height: '100%' }">
                    <v-row no-gutters>
                      <v-col cols="6" align="left">
                        <CardTitle>{{
                          $t('MES_CommLang.MES_CommLang_00427')
                        }}</CardTitle>
                        <!-- 가능설비목록 -->
                      </v-col>
                    ...
                    >
                      <KendoGrid
                        ...
                        @pageChangeEvent="pageChangeEvent3"
                      />
                    </div>
                  </CardBody>
                </Card>
    ...
    addBtn() {
      ...
      if (this.chkAllProduct) {
        ...
        if (this.selectedProcessID == null || this.selectedProcessID == '') {
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('MES_MsgLang.MES_MsgLang_00226')
          ); /* 알림, 공정을 선택해주세요. */
          return;
        }
      } else {
        if (
          ...
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('MES_MsgLang.MES_MsgLang_00227')
          ); /* 알림, 제품과 공정을 모두 선택해주세요. */
          return;
        }
      }
      ...
      if (delData.length > 0) {
        if (delData.filter(x => x.rowStat !== 'C').length >= 1) {
          ...
          this.$refs['confirmPop'].message =
            this.gridData3[0].PRODUCTID +
            ', ' +
            this.gridData3[0].PROCESSID +
            ' - ' +
