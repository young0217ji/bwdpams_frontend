<template>
  <div>
    <ol class="page-navigation">
      ...
      <!--기준정보-->
      <li>{{ $t('MES_CommLang.MES_CommLang_00272') }}</li>
      <!--제품라우팅설정-->
    </ol>
            ...
            <v-row no-gutters>
              <v-col :sm="9" :lg="10">
                <div class="form-group-wrap">
                  ...
                  <div class="form-group">
                    <label>{{ $t('MES_CommLang.MES_CommLang_00660') }}</label
                    ><!--설정여부-->
                    <Checkbox
                      ref="includeCheck"
                      :dataNm="'includeCheck'"
        ...
        <v-row :style="{ height: '100%' }">
            ...
            <Card ref="gridCard" :style="{ height: '100%' }">
              <CardBody :style="{ height: '100%' }">
                <v-col cols="6" align="left">
                  <CardTitle>{{
                    $t('MES_CommLang.MES_CommLang_00258')
                  }}</CardTitle
                  ><!--제품-->
                </v-col>
          ...
          >
            <Card ref="gridCard" :style="{ height: '100%' }">
              <CardBody :style="{ height: '100%' }">
                <CardTitle>{{
                  $t('MES_CommLang.MES_CommLang_00085')
                }}</CardTitle
                ><!--라우팅-->
            ...
            <Card :style="{ height: '100%' }">
              <CardBody :style="{ height: '100%' }">
                <v-row no-gutters>
                  <v-col cols="6" align="left">
                    <CardTitle>{{
                      $t('MES_CommLang.MES_CommLang_00520')
                    }}</CardTitle
                    ><!--제품-라우팅-->
                  </v-col>
...
export default {
  ...
  methods: {
    ...
    addBtn() {
      ...
      if (this.gridData.policy.length > 0) {
        this.$refs.alertPop.show(
          ...
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00122')
        ); /* 알림, 이미 라우팅이 연결된 품번입니다. */
        return;
      }
      const dataItem = {
        inEdit: true,
    ...
    async searchBtn() {
      ...
      await this.getGridData('product', 'GetRouteSetProd', params);
      await this.getGridData('routing', 'GetSelProcRoute', params);
      const poslicyParams = {
    ...
    delBtn() {
      ...
      if (hasChecked) {
        ...
        this.$refs['alertPop'].show(
          '',
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00164')
        ); /* 체크된 항목이 없습니다. */
      }
    },
    chkVal(nm, val) {
