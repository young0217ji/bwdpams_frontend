<template>
  <div>
    <ol class="page-navigation">
      ...
      <li>{{ $t("MES_CommLang.MES_CommLang_00546") }}</li> <!-- 도급업체작업자관리 -->
    </ol>
          ...
          <CardBody :style="{ width: '100%' }">
            <v-row>
              <v-col cols="6" align="left">
                <CardTitle>{{ $t("MES_CommLang.MES_CommLang_00547") }}</CardTitle> <!-- 작업자 목록 -->
              </v-col>
              <v-col cols="6" align="right">
                ...
                <Button :theme-color="'secondary'" :size="'small'" :icon="'delete'" @click="delBeforChk">{{
            ...
            <div ref="divWrapper" :style="{ height: 'calc(100% - 33px)' }">
              <KendoGrid ref="rowGrid" :gridHeight="gridHeight" :gridItems="gridData" :sortable="true" :gridPageData="gridPageData"
                ...
                :selected-field="selectedField" :gridDropDownList="headerParam.enumComboBox" :customCell="customCell"
                :isPaging="true" :checkHeaderArr="checkHeaderArr" @sortChangeHandler="sortChangeHandler"
    ...
    async saveBtn() {
      ...
      if (saveData.length > 0) {
        const res = this.mesPost({
          ...
          messagename: 'TxnSubContractUserMap',
          sendParam: saveData,
        }).then(result => {
          this.$nextTick(() => {
            this.getGridData();
    ...
    delBeforChk() {
      ...
      if (delData.length > 0) {
        if (delData.filter((x) => x.rowStat !== 'C').length >= 1) {
          ...
          this.$refs['confirmPop'].message = this.selected.WORKCENTER + ", " + this.selected.USERID + " - " + this.$i18n.t('MES_MsgLang.MES_MsgLang_00089'); // 선택한 데이터를 삭제 하시겠습니까?
