<template>
  <div>
    <ol class="page-navigation">
      ...
      <!-- 설비관리 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00594') }}</li>
      <!-- 예비품 기준정보 -->
    </ol>
          ...
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="6" align="left">
                <CardTitle>{{
                  $t('MES_CommLang.MES_CommLang_00591')
                }}</CardTitle>
                <!-- 예비품 목록 -->
              </v-col>
    ...
    saveBtn() {
      ...
      for (let i = 0; i < saveData.length; i++) {
        if (saveData[i].PARTID == null || saveData[i].PARTID == '') {
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('MES_MsgLang.MES_MsgLang_00235')
          ); /* 알림, 부품코드는 필수 입력 값 입니다. */
          return;
        }
      }
