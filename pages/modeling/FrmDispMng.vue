<template>
  <div>
    <ol class="page-navigation">
      ...
      <!-- 기준정보 -->
      <li>{{ $t('MES_CommLang.MES_CommLang_00727') }}</li>
      <!-- DISPLAY 현황판 관리 -->
    </ol>
    ...
    <v-row ref="contents">
      ...
      >
          ...
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="8" align="left">
                <CardTitle>{{
                  $t('MES_CommLang.MES_CommLang_00720')
                }}</CardTitle>
                <!-- 기본정보 -->
              </v-col>
        ...
        <CardTitle>{{ $t('MES_CommLang.MES_CommLang_00765') }}</CardTitle>
      </v-col>
      <v-col
        :sm="7"
        :lg="7"
          ...
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="1" align="left">
                <CardTitle>{{
                  $t('MES_CommLang.MES_CommLang_00765')
                }}</CardTitle>
                <!-- 기본정보 -->
              </v-col>
...
import SelectProcessModal from '@/components/frmDispMng/SelectProcessModal.vue';
import SelectEquipmentModal from '@/components/frmDispMng/SelectEquipmentModal.vue';
...
export default {
  ...
  methods: {
    ...
    MasterGridRowClick(event) {
      ...
      data.map(x => {
        return {
          ...x,
          selected:
            x.idx === event.dataItem.idx &&
            x.PROCESSID === event.dataItem.PROCESSID &&
            x.EQUIPMENTID === event.dataItem.EQUIPMENTID
              ? (x.selected = true)
              : (x.selected = false),
        };
      });
      ...
      this.mesGet({
        ...
        const data = res.map((x, idx) => {
          ...
          return {
            ...
            OriginalDownYN: !this.isNullOrEmpty(x.ORIGINALFILEPATH)
              ? true
              : false,
            OriginalUploadYN: true,
          };
      ...
      this.mesGet({
        ...
        if (data.length > 0) {
          ...
          this.getGridDetailData(
            this.gridData[0].PROCESSID,
            this.gridData[0].EQUIPMENTID
          );
    ...
    saveBtnDetail() {
      ...
      const fileList = [];
      this.gridDataFile
        .filter(x => !this.isNullOrEmpty(x.FILEPATH) && x.FILECHANGE)
        .map((x, idx) => {
          fileList.push({
            file: x.UPLOADFILE,
    ...
    gridButtonClick(e, field, dataItem) {
      ...
      this.selectedProcessId = this.gridData.filter(
        x => x.idx == this.btnClickIdx
      )[0].PROCESSID;
      this.selectedEquipmentId = this.gridData.filter(
        x => x.idx == this.btnClickIdx
      )[0].EQUIPMENTID;
      ...
      if (field === 'PROCESSID') {
        ...
        if (this.selectedProcessId == '' || this.selectedProcessId == null) {
          ...
          dataItem.PROCESSID.lastIndexOf('Monitoring') == 0 ||
          dataItem.PROCESSID.lastIndexOf(
            this.$i18n.t('MES_CommLang.MES_CommLang_00723')
          ) == 0
        ) {
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'), //알림
    ...
    selectedRow(row) {
      ...
      this.gridData = this.gridData.map(item => {
        return {
          ...item,
          PROCESSID:
            item.idx === this.btnClickIdx ? row.PROCESSID : item.PROCESSID,
          PROCESSNAME:
            item.idx === this.btnClickIdx ? row.PROCESSNAME : item.PROCESSNAME,
          EQUIPMENTID: item.idx === this.btnClickIdx ? '' : item.EQUIPMENTID,
          EQUIPMENTNAME:
            item.idx === this.btnClickIdx ? '' : item.EQUIPMENTNAME,
        };
      });
      ...
      if (
        row.PROCESSID.lastIndexOf('Monitoring') == 0 ||
        row.PROCESSID.lastIndexOf(
          this.$i18n.t('MES_CommLang.MES_CommLang_00723')
        ) == 0
      ) {
        ...
        this.gridData = this.gridData.map(item => {
          return {
            ...item,
            EQUIPMENTID:
              item.idx === this.btnClickIdx ? 'NONE' : item.EQUIPMENTID,
            EQUIPMENTNAME:
              item.idx === this.btnClickIdx ? 'NONE' : item.EQUIPMENTNAME,
          };
        });
        this.gridDataFile = this.gridDataFile.map(item => {
          return {
    ...
    selectedRowEquipment(row) {
      ...
      this.gridData = this.gridData.map(item => {
        return {
          ...item,
          EQUIPMENTID:
            item.idx === this.btnClickIdx ? row.EQUIPMENTID : item.EQUIPMENTID,
          EQUIPMENTNAME:
    ...
    griditemchange(e) {
      ...
      if (e.field === 'STARTTIME' || e.field === 'ENDTIME') {
        ...
        if (e.value.length > 4) {
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('MES_MsgLang.MES_MsgLang_00266')
          );
        }
        for (let i = 2; i <= 3; i++) {
          min[i - 2] = e.value[i];
    ...
    rowDdChange(e, name, dataItem) {
      ...
      if (this.gridDataFile) Data[idx] = { ...Data[idx], [name]: e.value };
    ...
    fileChange(e, field, dataItem) {
      ...
      const fileName = e.target.files[0].name.substring(
        0,
      ...
      this.gridDataFile = this.gridDataFile.map(item => {
        return {
          ...item,
          ORIGINALFILEPATH:
            item.idx === dataItem.idx
              ? '/프로젝트관리/98. 기타/09.개인폴더/이홍상/TEST/'.concat(
                  fileName.concat('.').concat(ext)
                )
              : item.ORIGINALFILEPATH,
          FILEPATH:
            item.idx === dataItem.idx
              ? 'D:\\BLWS\\FILE\\BLWS_IMAGE\\'.concat(
                  fileName.concat('.').concat(ext)
                )
              : item.FILEPATH,
          UPLOADFILE:
            item.idx === dataItem.idx ? e.target.files[0] : item.UPLOADFILE,
          FILECHANGE: item.idx === dataItem.idx ? true : item.FILECHANGE,
          rowStat:
            item.idx === dataItem.idx && dataItem.rowStat === 'N'
              ? 'U'
              : item.rowStat,
          SEQUENCE:
            item.idx === dataItem.idx ? dataItem.SEQUENCE : item.SEQUENCE,
          FILELOCATION:
            item.idx === dataItem.idx
              ? '\\\\172.31.15.21\\스마트 공장 Track A\\프로젝트관리\\98. 기타\\09.개인폴더\\이홍상\\TEST'
              : item.FILELOCATION,
    ...
    downloadBtn(e, field, item) {
      ...
      item.FILELOCATION =
        '///172.31.15.21/스마트 공장 Track A' +
        item.ORIGINALFILEPATH.substring(0, 31);
      window.open(
        '/api/mes/common/fileDownload?fileName=' +
          filename +
          '&fileLocation=' +
          item.FILELOCATION
      );
