<template>
    <div>
      <ol class="page-navigation">
        ...
        <li>{{ $t("MES_CommLang.MES_CommLang_00658") }}</li> <!-- AM라벨관리 -->
      </ol>
            ...
            <CardBody :style="{ width: '100%', height: '100%' }">
              <v-row no-gutters>
                <v-col cols="6" align="left">
                  <CardTitle>{{ $t("MES_CommLang.MES_CommLang_00659") }}</CardTitle> <!-- AM라벨목록 -->
                </v-col>
...
import { yammerBoxIcon } from '@progress/kendo-svg-icons';
  ...
  export default {
    ...
    methods: {
      ...
      saveBtn() {
        ...
        if (saveData.length > 0) {
          const res = this.mesPost({
            ...
            messagename: 'TxnUserAMProductDefinition',
            sendParam: saveData,
          }).then(result => {
            this.$nextTick(() => {
              this.getGridData();
      ...
      async getGridData() {
        ...
        this.gridData = data;
        const tempGridData = data;
        this.gridData.forEach(x => {
            tempGridData.forEach(y => {
                if(x.idx != y.idx && x.PLANTID == y.PLANTID && x.CUSTOMER == y.CUSTOMER && x.PRODUCTID == y.PRODUCTID){
                    x.disabled = false;
                    y.disabled = false;
                }
            })
        ...
        //     for(let j = 0; j < tempGridData.length; i++){
        //         if(this.gridData[i].PRODUCTID == tempGridData[j].PRODUCTID){
        //             this.gridData[i].disabled = false;
        //             this.gridData[j].disabled = false;
        //         }
        //     }
        // }
        });
