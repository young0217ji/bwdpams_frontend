<template>
  <div>
    <ol class="page-navigation">
      ...
      <li>{{ $t("MES_CommLang.MES_CommLang_00538") }}</li> <!-- 품번그룹정보 -->
    </ol>
              ...
              <v-col :sm="9" :lg="10">
                <div class="form-group-wrap">
                  <div class="form-group">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00541") }}</label> <!-- 제품그룹코드 -->
                    <InputText ref="productGroupId" :dataNm="'productGroupId'" :disabled="false" :searchOption="true"
                      :searchFunc="searchBtn" @input-text-set="searchInput" />
                  </div>
                  <div class="form-group">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00542") }}</label> <!-- 제품그룹명 -->
                    <InputText ref="productGroupName" :dataNm="'productGroupName'" :disabled="false" :searchOption="true"
                      :searchFunc="searchBtn" @input-text-set="searchInput" />
                  </div>
                  <div class="form-group" style="margin-top: 10px;">
                    <Label>{{ $t("MES_CommLang.MES_CommLang_00129") }}</Label> <!-- 사용여부 -->
                    <DropDownList ref="cmbActiveState" :style="{ width: '100px' }" :data-items="activeStateEnum"
                      :text-field="'text'" :data-item-key="'id'" :value="stateEnum" @change="comboChangeEnum">
                    </DropDownList>
                  </div>
                </div>
              </v-col>
          ...
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="6" align="left">
                <CardTitle>{{ $t("MES_CommLang.MES_CommLang_00539") }}</CardTitle> <!-- 제품그룹목록 -->
              </v-col>
  ...
  methods: {
    ...
    saveBtn() {
      ...
      if (saveData.length > 0) {
        const res = this.mesPost({
          ...
          messagename: 'TxnProductGroupDefinition',
          sendParam: saveData,
        }).then(result => {
          this.$nextTick(() => {
            this.getGridData();
    ...
    async getGridData() {
      ...
      this.mesGet({
        ...
        queryId: 'GetProductGroupGridList',
        sendParam: {
...
const data = list.map((x, idx) => {
    ...
    async getEnum() {
      this.mesGet({
        ...
        this.activeStateEnum.push({ id: '', text: this.$i18n.t('MES_CommLang.MES_CommLang_00501') });
        for (let i = 0; i < data.length; i++) {
          if (i < data.length) {
            this.activeStateEnum.push({ id: data[i].ENUMVALUE, text: data[i].ENUMVALUENAME });
