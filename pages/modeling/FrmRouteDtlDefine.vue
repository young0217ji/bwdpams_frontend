<template>
  <div>
    <ol class="page-navigation">
      ...
      <!--기준정보-->
      <li>{{ $t('MES_CommLang.MES_CommLang_00095') }}</li>
      <!--라우팅상세설정-->
    </ol>
            ...
            <v-row no-gutters>
              <v-col :sm="8" :lg="9">
                <div class="form-group-wrap" style="display: inline-block">
                  ...
                  <div
                    class="form-group"
                    style="margin-left: 138px; display: inline-block"
                  >
                    <div class="i-msg" style="color: red" v-if="confirm">
                      {{ $t('MES_MsgLang.MES_MsgLang_00173') }}
                    </div>
...
import SelectModelingConfirm from '@/components/frmRouteDtlDefine/SelectModelingConfirm';
import SelectRecipeParameter from '@/components/frmRouteDtlDefine/SelectRecipeParameter';
import RawMaterialInput from '@/components/frmRouteDtlDefine/RawMaterialInput';
import ProcessConditionSetting from '@/components/frmRouteDtlDefine/ProcessConditionSetting';
...
export default {
  ...
  data() {
    return {
      ...
      gridHeight2: '100px',
      gridHeight3:
        ((window.innerHeight || document.body.clientHeight) / 5) * 2 + 'px',
      gridHeight4:
        ((window.innerHeight || document.body.clientHeight) / 18) * 2 + 'px',
      gridHeight5:
        ((window.innerHeight || document.body.clientHeight) / 7) * 2 + 'px',
  ...
  methods: {
    ...
    async searchBtn(data) {
      ...
      if (Object.keys(data).length === 0) {
        ...
        this.$refs.alertPop.message =
          '라우팅 상세설정을 할 제품정보를 선택해 주시기 바랍니다.';
      ...
      await this.mesGet({
        ...
        queryId: 'GetConsumeCondtMatList',
        sendParam: {
    ...
    async initTree(reset) {
      ...
      let res = await this.mesGet({
        apiKey: 'mes/common/getqueryresult',
        // 조수완 : GetConsumableConditionSetTree 에서 수정
        queryId: 'GetConsumeCondtSetTree',
        sendParam: {
          plantid: this.$auth.$state.user.plantId,
      ...
      if (!reset) {
        this.gridData2
          .filter(x => x.ROCCOMPOSITIONID !== null)
          .map((x, idx) => {
            res.unshift({
      ...
      await this.$refs.RawMaterialInput.getGridData(
        this.gridData2,
        this.clickTreeData
      );
      if (this.v1) {
        await this.$refs.ProcessConditionSetting.getGridData(
          this.gridData3,
          this.clickTreeData
        );
      }
    ...
    onItemClick(event) {
      ...
      if (this.clickTreeData.item.LEVELVALUE === 'CONSUMABLE') {
        ...
        this.$refs.RawMaterialInput.gridData
          ...
          .map(x => {
            if (
              this.clickTreeData.item.UIKEY === x.ROCCOMPOSITIONID &&
              this.clickTreeData.item.PROCESSID === x.CONSUMABLEID
            ) {
              this.$refs.RawMaterialInput.gridData[x.idx].selected =
                !this.$refs.RawMaterialInput.gridData[x.idx].selected;
            }
          });
      }
    ...
    leftBtn() {
      ...
      if (this.selected === 0) {
        if (this.clickTreeData.item.LEVELVALUE === 'RECIPE') {
          if (
            this.$refs.RawMaterialInput.gridData.filter(
              x => x.selected === true
            ).length === 0
          ) {
                ...
                .forEach((x, idx) => {
                  ...
                  if (
                    ...
                    !this.gfn_ChkOriginalData(
                      this.$refs.RawMaterialInput.gridHeader,
                      this.$refs.RawMaterialInput.gridData,
                      this.$refs.RawMaterialInput.gridOriData,
                      idx
                    )
                  ) {
                    x.rowStat = 'N';
    ...
    rightBtn() {
      ...
      if (this.selected === 0) {
        if (this.clickTreeData.item.LEVELVALUE === 'CONSUMABLE') {
          this.$refs.RawMaterialInput.gridData.forEach((x, idx) => {
          ...
          for (let i = 0; i < this.treeData[0].children.length; i++) {
            for (
              let x = 0;
              x < this.treeData[0].children[i].children.length;
              x++
            ) {
              for (
                let z = 0;
                z < this.treeData[0].children[i].children[x].children.length;
                z++
              ) {
                if (
                  this.treeData[0].children[i].children[x].children[z] !==
                  undefined
                ) {
                  if (
                    this.treeData[0].children[i].children[x].children[z]
                    ...
                    this.treeData[0].children[i].children[x].children.splice(
                      z,
                      1
                    );
                    z--;
    ...
    saveBtn() {
      ...
      if (this.clickTreeData.item !== undefined) {
        let compareData = this.gridData3
          .filter(
            x =>
              x.COMPOSITIONID === this.clickTreeData.item.COMPOSITIONID &&
              x.rowStat === 'N'
          )
          .map(x => {
            // RECIPEPARAMETERID
    ...
    addBtn() {
      ...
      if (
        this.clickTreeData.item.LEVELVALUE === 'RECIPE' ||
        this.clickTreeData.item.LEVELVALUE === 'PROCESS'
      ) {
        if (this.clickTreeData.item.RECIPETYPE === 'Consumable') {
          this.$refs['alertPop2'].show(
            '',
            '스텝 원료투입항목에서는 공정조건을 추가 할 수 없습니다.'
          );
          return;
        } else if (this.clickTreeData.item.RECIPETYPE === 'End') {
          this.$refs['alertPop2'].show(
            '',
            '스텝 종료항목에서는 공정조건을 추가 할 수 없습니다.'
          );
          return;
        } else {
    ...
    addData(data) {
      ...
      for (let i = 0; i < this.treeData[0].children.length; i++) {
        if (this.clickTreeData.itemHierarchicalIndex.length === 5) {
          if (
            this.treeData[0].children[i].COMPOSITIONID ===
            this.clickTreeData.item.UIKEY
          ) {
            for (
              let x = 0;
              x < this.treeData[0].children[i].children.length;
              ...
              if (
                this.treeData[0].children[i].children[x].LEVELVALUE ===
                  'RECIPE' &&
                this.treeData[0].children[i].children[x].COMPOSITIONID ===
                  this.clickTreeData.item.COMPOSITIONID
              ) {
                for (
                  let z = 0;
                  z < this.treeData[0].children[i].children[x].children.length;
                ...
                saveData.map((y, idx) => {
                  ...
                  this.treeData[0].children[i].children[x].children.push(
                    tmpTreeData
                  );
                });
              }
    ...
    delBtn() {
      ...
      const data = JSON.parse(
        JSON.stringify(this.$refs.ProcessConditionSetting.gridData)
      );
        ...
        for (let y = 0; y < delData.length; y++) {
          for (
            ...
            if (this.treeData[0].children[i].children[x] !== undefined) {
              if (
                this.treeData[0].children[i].children[x].children.length > 0
              ) {
                for (
                  let z = 0;
                  z < this.treeData[0].children[i].children[x].children.length;
                  ...
                  if (
                    this.treeData[0].children[i].children[x].children[z]
                      .PROCESSID === delData[y].RECIPEPARAMETERID &&
                    this.treeData[0].children[i].children[x].children[z]
                      .UIKEY === delData[y].COMPOSITIONID
                  ) {
                    this.treeData[0].children[i].children[x].children.splice(
                      z,
                      1
                ...
                if (
                  this.treeData[0].children[i].children[x].PROCESSID ===
                    delData[y].RECIPEPARAMETERID &&
                  this.treeData[0].children[i].children[x].UIKEY ===
                    delData[y].COMPOSITIONID
                ) {
                  this.treeData[0].children[i].children.splice(x, 1);
                  x--;
                }
    ...
    undoBtn() {
      this.clickTreeData = {};
      this.$refs.ProcessConditionSetting.gridData = [];
      this.$refs.ProcessConditionSetting.gridData = JSON.parse(
        JSON.stringify(this.$refs.ProcessConditionSetting.gridOriData)
      );
