<template>
    <div>
      <ol class="page-navigation">
        ...
        <li>{{ $t("MES_CommLang.MES_CommLang_00933") }}</li><!--Schedule Chart-->
      </ol>
            ...
            <CardBody :style="{ width: '100%' }">
              <v-row>
                <v-col cols="6" align="left">
                  <CardTitle>{{ $t("MES_CommLang.MES_CommLang_00933") }}</CardTitle><!--Schedule Chart-->
                </v-col>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rowHeaderColumns: [
          ...
          { text: this.$i18n.t('MES_CommLang.MES_CommLang_00354'), display:'DUEDATE'}, // 날짜
          { text: this.$i18n.t('MES_CommLang.MES_CommLang_00934'), display:'DAYSEQUENCE'}, // 순서
    ...
    methods: {
      ...
      async getGridData(){
        ...
        this.mesGet({
          apiKey: "mes/common/customquery",
          queryId: "GetScheduleGanttChartList",
          sendParam: {
            plantid: this.$auth.$state.user.plantId,
          ...
          res.map((x, idx)=>{
            ...
            if(eventList.filter(y=> this.findEventRow(x,y)).length==0){
              eventList.push({
                ...
                WORKCENTER: x.WORKCENTER,
                id:idx, 
                start:this.StringToDate(x.DUEDATE), 
                end:this.StringToDate(x.DUEDATE),
      ...
      findResourceRow(data, resource){
        return data.DUEDATE == resource.tags.DUEDATE
            && data.DAYSEQUENCE == resource.tags.DAYSEQUENCE
      ...
      StringToDate(String){
        ...
        resultDate.setMonth(Number(String.substring(5,7))-1);
        resultDate.setDate(String.substring(8,10));
