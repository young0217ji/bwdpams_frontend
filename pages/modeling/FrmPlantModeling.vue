<template>
  <div>
    <ol class="page-navigation">
      ...
      <li>{{ $t("MES_CommLang.MES_CommLang_00049") }}</li> <!-- 공장기본설정 -->
    </ol>
  ...
  methods: {
    ...
    saveValidation() {
      ...
      if (validation.length > 0) {
        this.openAlert(this.$i18n.t('MES_CommLang.MES_CommLang_00414'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00001'));
        return;
      }
      ...
      if (dupArr.length > 0) {
        this.openAlert(this.$i18n.t('MES_CommLang.MES_CommLang_00414'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00002'));
        return;
      }
      this.saveBtn();
    },
    async saveBtn() {
      ...
      if (saveData.length > 0) {
        ...
        if (res?.returncode == '0') {
          this.openAlert(this.$i18n.t('MES_CommLang.MES_CommLang_00414'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00140'));
          this.initTree();
          this.getGridData(this.parentAreaId);
        } else {
          this.openAlert('error', this.$i18n.t('MES_MsgLang.MES_MsgLang_00109'));
        }
      } else {
        this.openAlert(this.$i18n.t('MES_CommLang.MES_CommLang_00414'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00143'));
      }
    },
    //그리드 데이터 가져오기
    ...
    rowDdChange(e, name, dataItem) {
     ...
     selectedRow(row) {
      ...
      .map(item => {
        return {
          ...item,
          REPRESENTATIVEPROCESSID: item.idx === this.btnClickIdx ? row.PROCESSID : item.REPRESENTATIVEPROCESSID,
    ...
    delConfirm() {
      ...
      this.$refs.ConfirmPop.message = `${chk.join()}` + " - " + this.$i18n.t('MES_MsgLang.MES_MsgLang_00089'); // 선택한 데이터를 삭제 하시겠습니까?
