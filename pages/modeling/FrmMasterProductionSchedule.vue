<template>
  <div>
    <ol class="page-navigation">
      ...
      <li>{{ $t("MES_CommLang.MES_CommLang_00616") }}</li> <!-- 용접기준정보 -->
    </ol>
              ...
              <v-col :sm="9" :lg="10">
                <div class="form-group-wrap">
                  <div class="form-group">
                    <label>{{ $t("MES_CommLang.MES_CommLang_00542") }}</label> <!-- 품번그룹명 -->
        ...
        <Card ref="gridCard" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%', height: '100%' }">
            ...
            <div ref="divWrapper" :style="{ height: '100%' }">
              <KendoGrid ref="productList" :gridHeight="gridHeight" :gridItems="gridData" :sortable="true"
                :columns="listParam.gridHeader" :resizable="true" :selected-field="selectedField" :isPaging="true"
                :gridPageData="gridPageData"
                :reSetPage="this.reSetPage" @gridrowclick="ProductDefinitionRowClick"
        ...
        <Card ref="girdCard" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%', height: '100%' }">
            <v-row no-gutters>
              <v-col cols="12" style="margin-bottom: 10px;" align="right">
                ...
                <kbutton :theme-color="'secondary'" :size="'small'" :icon="'add'" @click="addBtn"
                  :disabled="disableAddBtn">{{ $t("MES_CommLang.MES_CommLang_00411") }}</kbutton> <!-- 추가 -->
            ...
            <div ref="divWrapper2" style="height: calc(100% - 33px)">
              <KendoGrid ref="weldingGrid" :gridHeight="gridHeight" :gridItems="gridDataWelding" :sortable="true"
                :columns="headerParam.gridHeader" :resizable="true" :edit-field="'inEdit'" :selected-field="selectedField"
                :isPaging="true" :reSetPage="this.reSetPage" @selectionchange="onSelectionchange"
                :gridPageData="gridPageDataMaster"
                @gridrowclick="onRowClick" @griditemchange="griditemchange" :gridDropDownList="headerParam.enumComboBox"
  ...
  methods: {
    ...
    ProductDefinitionRowClick(event) {
      ...
      data.map(x => {
        return {
          ...x,
          selected: x.PRODUCTID === event.dataItem.PRODUCTID ? x.selected = true : x.selected = false,
        }
      });
    ...
    saveBtn() {
      ...
      const validate = this.mesValidation(this.headerParam.gridHeader, this.gridDataWelding);
    ...
    griditemchange(e) {
      ...
      if (
        data[idx].rowStat === 'U' &&
        !this.gfn_ChkOriginalData(this.headerParam.gridHeader, data, this.gridOriDataWelding, idx)
      ) {
        data[idx].rowStat = 'N';
      }
   ...
   rowDdChange(e, name, dataItem) { 
    ...
    //unbo 버튼 클릭
    undoBtn() {
      this.gridDataWelding = this.gridOriDataWelding.filter((x) => x.newRow !== true);
      this.$nuxt.$emit('iccReset');
    },
  },
};
