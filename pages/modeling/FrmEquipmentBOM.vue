<template>
  <div>
    <ol class="page-navigation">
      ...
      <li>{{ $t("MES_CommLang.MES_CommLang_00074") }}</li> <!-- 기준정보홈 -->
      <li>{{ $t("MES_CommLang.MES_CommLang_00180") }}</li> <!-- 공장기본설정 -->
    </ol>
        ...
        <Card ref="gridCard" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%' }">
            ...
            <div ref="divWrapper" :style="{ height: '96%' }">
              <KendoTree :treeData="treeData" :textField="'AREANAME'" :icon="'icon'" :activeItem="treeActiveItem"
        ...
        <Card ref="gridCard" :style="{ height: '100%' }">
          <CardBody :style="{ width: '100%' }">
            <v-row no-gutters style="margin-bottom: -7px;">
              ...
              <v-col cols="12" align="right">
                <Button v-show="saveBtnVisible" :theme-color="'primary'" :size="'small'" :icon="'save'"
                  @click="saveBtn">{{
                    $t("MES_CommLang.MES_CommLang_00414") }}</Button> <!-- 저장 -->
                <Button v-show="addBtnVisible" :theme-color="'secondary'" :size="'small'" icon="add" @click="addBtn">{{
                  $t("MES_CommLang.MES_CommLang_00411") }}</Button> <!-- 추가 -->
                <Button v-show="delBtnVisible" :theme-color="'secondary'" :size="'small'" icon="delete"
                  @click="confirmDel">{{
            ...
            <v-row no-gutters>
              <v-col cols="12" align="left">
                <CardTitle style="margin-bottom: -10px;">{{
                  $t("MES_CommLang.MES_CommLang_00180") }}</CardTitle> <!-- 설비기준정보 -->
              </v-col>
            </v-row>
                ...
                <v-row no-gutters>
                  <v-col :cols="12">
                    <div class="form-group-wrap" style="margin-bottom: 5px;">
                      ...
                      <div class="form-group">
                        <label>{{
                          $t("MES_CommLang.MES_CommLang_00177") }}</label> <!-- 설비코드 -->
                        <InputText ref="equipmentid" :dataNm="'equipmentid'" :disabled="false" :readonly="inputReadOnly"
                          :searchOption="true" @input-text-set="searchInput" />
                      </div>
                      <div class="form-group">
                        <label>{{
                          $t("MES_CommLang.MES_CommLang_00171") }}</label> <!-- 설비명 -->
                        <InputText ref="equipmentName" :dataNm="'equipmentName'" :disabled="false"
                          :readonly="inputReadOnly" :searchOption="true" @input-text-set="searchInput" />
                      </div>
                      <div class="form-group" style="justify-content: right;">
                        <label>설비 이미지</label>
                        <!-- <input type="file" ref="fileInput" style="display: none" @change="handleFileChange" /> -->
                        <InputText ref="fileName" :dataNm="'fileName'" :disabled="inputReadOnly" :readonly="inputReadOnly"
                          :searchOption="true" @input-text-set="searchInput" :style="{width:'200px'}"/>
                    ...
                    <div class="form-group-wrap" style="margin-bottom: 5px;">
                      ...
                      <div class="form-group">
                        <label>{{
                          $t("MES_CommLang.MES_CommLang_00176") }}</label> <!-- 설비구분 -->
                        <DropDownList ref="cmbEquipmentType" :style="{ width: '165px' }" :data-items="equipmentType"
                          :disabled="false" :text-field="'text'" :data-item-key="'id'" :value="equipmentTypeValue"
                          @change="changeEquipmentType">
                        </DropDownList>
                      </div>
                      <div class="form-group">
                        <label>설비상세</label> <!-- 설비상세 -->
                        <DropDownList ref="cmbEquipmentDetail" :style="{ width: '165px' }" :data-items="equipmentDetail"
                          :disabled="false" :text-field="'text'" :data-item-key="'id'" :value="equipmentDetailValue"
                          @change="changeEquipmentDetail">
                        </DropDownList>
                      </div>
                      <div class="form-group">
                        <label>설비등급</label> <!-- 설비등급 -->
                        <DropDownList ref="cmbEquipmentGrade" :style="{ width: '200px' }" :data-items="equipmentGrade"
                          :disabled="false" :text-field="'text'" :data-item-key="'id'" :value="equipmentGradeValue"
                    ...
                    <div class="form-group-wrap" style="margin-bottom: 5px; ">
                      ...
                      <div class="form-group">
                        <label>사용상태</label> <!-- 사용상태 -->
                        <DropDownList ref="cmbUseflag" :style="{ width: '165px' }" :data-items="activestate"
                          :disabled="false" :text-field="'text'" :data-item-key="'id'" :value="activestateValue"
                          @change="changeUseflag">
                        </DropDownList>
                      </div>
                      <div class="form-group">
                        <label>설비 SPEC</label> <!-- 설비 SPEC -->
                        <InputText ref="equipmentSpec" :dataNm="'equipmentSpec'" :disabled="false"
                          :readonly="inputReadOnly" :searchOption="true" @input-text-set="searchInput" />
                      </div>
                    </div>
                  </v-col>
            ...
            <v-row no-gutters>
              <v-col cols="12" align="left">
                <CardTitle style="margin-bottom: -10px;">{{
                  $t("MES_CommLang.MES_CommLang_00768") }}</CardTitle> <!-- 설비기준정보 -->
              </v-col>
            </v-row>
                  ...
                  <v-col :cols="12">
                    <div class="form-group-wrap" style="margin-bottom: 5px;">
                      <div class="form-group">
                        ...
                        <InputText ref="accetno" :dataNm="'accetno'" :disabled="false" :readonly="inputReadOnly"
                          :searchOption="true" />
                      </div>
                      <div class="form-group" style="justify-content: right;">
                        <label>내용년수</label>
                        <InputText ref="assetYear" :dataNm="'assetYear'" :disabled="false" :readonly="inputReadOnly"
                          :searchOption="true" @input-text-set="searchInput" />
                      </div>
                      <div class="form-group">
                        <label>{{
                          $t("MES_CommLang.MES_CommLang_00775") }}</label> <!-- 제작회사 -->
                        <InputText ref="maker" :dataNm="'maker'" :disabled="false" :readonly="inputReadOnly"
                          :searchOption="true" @input-text-set="searchInput" />
                      </div>
                    </div>
                    <div class="form-group-wrap" style="margin-bottom: 5px;">
                      ...
                      <div class="form-group" style="margin-left: 5px;">
                        <label>메모</label> <!-- 메모 -->
                        <InputText ref="memo" :dataNm="'memo'" :disabled="false" :readonly="inputReadOnly"
                          :searchOption="true" @input-text-set="searchInput" />
                      </div>
                      <div class="form-group">
                        <label>{{
                          $t("MES_CommLang.MES_CommLang_00770") }}</label> <!-- 취득가액 -->
                        <InputText ref="acquisitionAmount" :dataNm="'acquisitionAmount'" :disabled="false"
                          :readonly="false" :searchOption="true" @input-text-set="searchInput" />
                      </div>
                      <div class="form-group">
                        <label>{{
                    ...
                    <div class="form-group-wrap" style="margin-bottom: 5px; ">
                      ...
                      <div class="form-group" style="margin-left: 5px;">
                        <label>취득일자</label> <!-- 취득일자 -->
                        <InputText ref="acquisitionTime" :dataNm="'acquisitionTime'" :disabled="false"
                          :readonly="inputReadOnly" :searchOption="true" @input-text-set="searchInput" />
                      </div>
                      <div class="form-group">
                        <label>취득근거번호</label> <!-- 취득근거번호 -->
                        <InputText ref="acquisitionReasoneNo" :dataNm="'acquisitionReasoneNo'" :disabled="false"
                          :readonly="inputReadOnly" :searchOption="true" @input-text-set="searchInput" />
                      </div>

                    </div>
                    <div class="form-group-wrap" style="margin-bottom: 5px; margin-left: 5px;">
                      <div class="form-group">
                        <label>폐기일자</label> <!-- 폐기일자 -->
                        <InputText ref="deposeTime" :dataNm="'deposeTime'" :disabled="false" :readonly="inputReadOnly"
                          :searchOption="true" @input-text-set="searchInput" />
                      </div>
                      <div class="form-group">
                        <label>폐기근거번호</label> <!-- 폐기근거번호 -->
                        <InputText ref="deposeReasoneNo" :dataNm="'deposeReasoneNo'" :disabled="false"
                          :readonly="inputReadOnly" :searchOption="true" @input-text-set="searchInput" />
                      </div>
                    </div>
                  </v-col>
            ...
            <v-col :cols="12" :class="'divList1'" style="margin-top: 8px; height: 25%;">
              <v-row no-gutters>
                <v-col cols="6" align="left">
                  <CardTitle style="margin-bottom: 10px;">하위설비 리스트</CardTitle> <!-- 계측기 상태관리 -->
                </v-col>
                <v-col cols="6" align="right">
                  <Button v-show="gridBtnShow" :theme-color="'secondary'" :size="'small'" icon="add" @click="addBtn2">{{
                    $t("MES_CommLang.MES_CommLang_00411") }}</Button> <!-- 추가 -->
                  <Button v-show="gridBtnShow" :theme-color="'secondary'" :size="'small'" icon="delete"
                    @click="confirmDel2">{{
                      $t("MES_CommLang.MES_CommLang_00412") }}</Button> <!-- 삭제 -->
                  <Button v-show="gridBtnShow" :theme-color="'secondary'" :size="'small'" :icon="'undo'"
              ...
              <div ref="divWrapper" style="height: 100%;">
                <KendoGrid ref="rowGrid" :gridHeight="gridHeight" :gridItems="gridData" :sortable="true"
                  ...
                  :gridDropDownList="headerParam.enumComboBox" :selected-field="selectedField" @gridrowclick="onRowClick"
                  @griditemchange="griditemchange" @gridddchang="rowDdChange" />
              </div>
...
export default {
  ...
  methods: {
    ...
    handleFileChange(event) {
      ...
      this.$refs.fileName.textVal = this.fileTimestamp(event.target.files[0].name);
      this.fileName = this.fileTimestamp(event.target.files[0].name);
      this.uploadFile = event.target.files[0];
      this.fileLocation = "/프로젝트관리/98. 기타/09.개인폴더/이홍상/TEST/".concat(this.fileName);
    ...
    async getComboList() {
      this.mesGet({
        ...
        for (let i = 0; i < data.length; i++) {
          if (i < data.length) {
            this.equipmentType.push({ id: data[i].ENUMVALUE, text: data[i].ENUMVALUENAME });
          }
        }
      });
      this.mesGet({
        ...
        for (let i = 0; i < data.length; i++) {
          if (i < data.length) {
            this.equipmentDetail.push({ id: data[i].ENUMVALUE, text: data[i].ENUMVALUENAME });
          }
        }
      });
      ...
      this.mesGet({
        ...
        for (let i = 0; i < data.length; i++) {
          if (i < data.length) {
            this.activestate.push({ id: data[i].ENUMVALUE, text: data[i].ENUMVALUENAME });
          }
        }
      });
    },
    ...
    async initTree() {
      ...
      const res = await this.mesGet({
        apiKey: 'mes/common/getqueryresult',
        queryId: 'GetAreaListForEquipment',
        queryVersion: '00001',
        sendParam: {
          SUPERAREAID: '',
          plantid: this.$auth.$state.user.plantId,
    ...
    onItemClick(event) {
      ...
      if (event.item.AREATYPE === "WorkCenter") {
        ...
        this.equipmentTypeValue = this.equipmentType.filter(x => x.id == event.item.EQUIPMENTTYPE)[0]
        this.equipmentDetailValue = this.equipmentDetail.filter(x => x.id == event.item.EQUIPMENTDETAILTYPE)[0]
        this.activestateValue = this.activestate.filter(x => x.id == event.item.ACTIVESTATE)[0];
