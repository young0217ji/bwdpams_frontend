  ...
  methods: {
    ...
    rowdblclick(event) {
      for (let i = 0; i < this.masterData.length; i++) {
        if (
          this.masterData[i].PROCESSID == this.processId &&
          event.dataItem.EQUIPMENTID == this.masterData[i].EQUIPMENTID
        ) {
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('MES_CommLang.MES_CommLang_00053') +
              ': ' +
              this.processId +
              ' ' +
              this.$i18n.t('MES_CommLang.MES_CommLang_00177') +
              ': ' +
              event.dataItem.EQUIPMENTID +
              this.$i18n.t('MES_MsgLang.MES_MsgLang_00269')
          );
          return;
        }
      }
    ...
    selectRow() {
      ...
      for (let i = 0; i < this.masterData.length; i++) {
        if (
          this.masterData[i].PROCESSID == this.processId &&
          returnData[0].EQUIPMENTID == this.masterData[i].EQUIPMENTID
        ) {
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('MES_CommLang.MES_CommLang_00053') +
