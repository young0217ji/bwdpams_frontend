...
export default {
  ...
  methods: {
    ...
    rowdblclick(event) {
      if (
        event.dataItem.PROCESSID.lastIndexOf('Monitoring') == 0 ||
        event.dataItem.PROCESSID.lastIndexOf(
    ...
    selectRow() {
      ...
      if (
        returnData[0].PROCESSID.lastIndexOf('Monitoring') == 0 ||
        returnData[0].PROCESSID.lastIndexOf(
          this.$i18n.t('MES_CommLang.MES_CommLang_00723')
        ) == 0
      ) {
        for (let i = 0; i < this.masterData.length; i++) {
