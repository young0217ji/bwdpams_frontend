      ...
      <v-row ref="contents">
        ...
        <v-col :cols="1" :style="{ height: '100%'}" align="center">
          <div class="between-opt-align" :style="{ height: '100%'}">
            <div>
              <Button :theme-color="'secondary'" :icon="'arrow-chevron-right'" @click="addBtn">{{ $t("MES_CommLang.MES_CommLang_00411") }}</Button> <!-- 추가 -->
            </div>
            <div>
...
export default { 
  ...
  async mounted() {
    this.sourceHeader = await this.getCustomHeader("dgvOperationPop",true,true)
    this.getComboData();
    this.getGridData();
  },
  methods: {
