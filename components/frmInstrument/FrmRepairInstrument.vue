<template>
    <div>
        <Dialog v-if="visibleDialog" :title="$t('MES_CommLang.MES_CommLang_00739')"  width="55%" height="250" @close="toggleDialog">
            <v-row no-gutters style="margin-bottom: -7px;">
                <v-col cols="6" align="left">
                    <CardTitle style="margin-bottom: -10px;"> {{ $t("MES_CommLang.MES_CommLang_00739") }}</CardTitle><!-- 수리실적 등록 -->
                </v-col>
                            ...
                            <v-row no-gutters>
                                <v-col :cols="12">
                                    <div class="form-group-wrap" style="margin-bottom: 5px;">
                                        ...
                                        <div class="form-group">
                                            <label> {{ $t("MES_CommLang.MES_CommLang_00738") }}</label> <!-- 수리업체 -->
...
export default {
    ...
    async mounted() {
        this.$refs['actiontype'].textVal = this.$i18n.t('MES_CommLang.MES_CommLang_00785'); // 수리
            ...
            const res = this.mesPost({
                ...
                this.$nextTick(() => {
                    if (result.returncode !== undefined && result.returncode === '0') {
                        ...
                        this.$emit('repairinstrumentDetail', true)
                        this.$emit('visibleDialog', !this.visibleDialog)
                    } else {
