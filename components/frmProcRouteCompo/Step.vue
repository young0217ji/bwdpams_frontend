  ...
  <div :style="{ height: '100%' }">
    <v-row no-gutters>
      <v-col cols="8" align="left">
        <CardTitle>{{ $t('MES_CommLang.MES_CommLang_00193') }}</CardTitle>
        <!-- 스텝 -->
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    ...
    rowDdChange(e, name, dataItem) {
      const idx = this.gridData.findIndex(
        x => x.RECIPESEQUENCE === dataItem.RECIPESEQUENCE
      );
    ...
    gridButtonClick(e, field, dataItem) {
      if (field === 'CONCURRENCYPROCESSBUTTON') {
        this.currentGridData = this.gridData.filter(x => x.idx > dataItem.idx);
