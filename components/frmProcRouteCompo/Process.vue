  ...
  <div :style="{ height: '100%' }">
    <v-row no-gutters>
      <v-col cols="5" align="left">
        <CardTitle>{{ $t('MES_CommLang.MES_CommLang_00052') }}</CardTitle>
        <!--공정-->
      </v-col>
      <v-col cols="7" align="right">
        ...
        <Button
          ...
          :disabled="!isTreeClicked"
          @click="upBtn"
          >{{ $t('MES_CommLang.MES_CommLang_00443') }}</Button
        >
        <!-- 위로 -->
        <Button
          :theme-color="'secondary'"
          ...
          :disabled="!isTreeClicked"
          @click="downBtn"
          >{{ $t('MES_CommLang.MES_CommLang_00442') }}</Button
        >
        <!-- 아래로 -->
        ...
        <Button
          ...
          >{{ $t('MES_CommLang.MES_CommLang_00058') }}</Button
        >
        <!-- 공정삽입 -->
        <Button
          :theme-color="'primary'"
          ...
          @click="insertStep"
          >{{ $t('MES_CommLang.MES_CommLang_00196') }}</Button
        >
        <!-- 스텝삽입 -->
        <Button
          :theme-color="'primary'"
          :size="'small'"
          @click="getRoutingSet"
          >{{ $t('MES_MsgLang.MES_MsgLang_00043') }}</Button
        >
        <!-- 라우팅 구성 가져오기 -->
      </v-col>
    </v-row>
...
import RoutingSetModal from '@/components/frmProcRouteCompo/RoutingSetModal';
import InsertStep from '@/components/frmProcRouteCompo/InsertStep';
import InsertOperation from '@/components/frmProcRouteCompo/InsertOperation';
...
export default {
  ...
  methods: {
    ...
    routingSet({ process, step, tree }) {
      const pData = process.map(x => ({
        ...x,
        DESCRIPTIONSET: this.$i18n.t('MES_CommLang.MES_CommLang_00287'),
      })); // 지시사항등록
      const sData = step.map(x => ({
        ...x,
    ...
    saveBtnBefore() {
      ...
      if (bFlag) {
        ...
        this.$refs.alertPop.message =
          this.headerParam.gridHeader.filter(
            item => item.field == 'ERPPROCESSCODE'
          )[0].title +
          ' - ' +
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00016'); // 값은 필수 입력 항목입니다.
        this.$refs.alertPop.modalWidth = '330px';
        this.$refs.alertPop.visibleDialog = true;
        return;
      }
    ...
    insertOperation() {
      if (this.isTreeChange) {
        this.openAlert(
          this.$i18n.t('MES_CommLang.MES_CommLang_00052'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00270')
        ); // 공정, 트리변경시 저장후 가능합니다.
        return;
      }
    ...
    insertStep() {
      if (this.isTreeChange) {
        this.openAlert(
          this.$i18n.t('MES_CommLang.MES_CommLang_00193'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00270')
        ); // 스텝, 트리변경시 저장후 가능합니다.
        return;
      }
    ...
    delBtn() {
      ...
      if (this.isTreeChange) {
        this.openAlert(
          this.$i18n.t('MES_CommLang.MES_CommLang_00412'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00270')
        ); // 삭제, 트리변경시 저장후 가능합니다.
        return;
      }
      ...
      if (flag.length === 3) {
        //스텝
        if (
          delTarget.RECIPETYPE == 'Change' ||
          delTarget.RECIPETYPE == 'Consumable'
        ) {
          const step = this.dgvRecipe.filter(
            x => x.RECIPERELATIONCODE !== delTarget.RECIPERELATIONCODE
          );
          this.setDgvRecipeAction(step);
          this.mkTree(this.frmPRC_Data, this.dgvOperation, step);
        } else {
          this.$refs.ConfirmPop.title = this.$i18n.t(
            'MES_CommLang.MES_CommLang_00409'
          ); // 확인
          this.$refs.ConfirmPop.message =
            `${delTarget.ALIASRECIPENAME || delTarget.PROCESSSEQUENCE}` +
            this.$i18n.t('MES_MsgLang.MES_MsgLang_00050') +
            this.$i18n.t('MES_MsgLang.MES_MsgLang_00271'); //를 삭제하겠습니까?, 삭제시 같은 스텝들은 전부 삭제 됩니다.
        ...
        this.$refs.ConfirmPop.message = this.$i18n.t(
          'MES_MsgLang.MES_MsgLang_00272'
        ); // 삭제할 경우 스텝들도 같이 삭제 됩니다. 삭제하시겠습니까?"
        this.$refs.ConfirmPop.modalWidth = '330px';
        this.$refs.ConfirmPop.visibleDialog = true;
    ...
    delProcess() {
      ...
      const process = this.dgvOperation.filter(x => {
        if (
          !(
            x.PROCESSID == delTarget.PROCESSID &&
            x.PROCESSSEQUENCE == delTarget.PROCESSSEQUENCE
          )
        ) {
          return x;
        }
      ...
      const step = this.dgvRecipe.filter(
        x => x.PROCESSSEQUENCE !== delTarget.PROCESSSEQUENCE
      );
    ...
    initDgvs() {
      ...
      this.treeData[0].items.forEach((x, idx) => {
        ...
        arrStep = arrStep.concat(
          x.items.map(y => ({ ...y, PROCESSSEQUENCE: PROCESSSEQUENCE }))
        );
      });
    ...
    upBtn() {
      ...
      if (flag.length === 3) {
        ...
        const newProcess = arrayMoveImmutable(
          this.dgvOperation,
          targetIdx,
          targetIdx - 1
        );
    ...
    downBtn() {
      ...
      if (flag.length === 3) {
        ...
        let newProcess = arrayMoveImmutable(
          this.dgvOperation,
          targetIdx,
          Number(targetIdx) + 1
        );
    ...
    mvStep(targetIdx, moveValue, type) {
      ...
      const sameProcessStepList = this.dgvRecipe.filter(
        x => x.PROCESSSEQUENCE == nowProcessSeq
      );
      const nowStepIndex = this.dgvRecipe.findIndex(
        x => x.RECIPESEQUENCE == moveTarget.RECIPESEQUENCE
      );
      const nowSameStepIndex = sameProcessStepList.findIndex(
        x => x.RECIPESEQUENCE == moveTarget.RECIPESEQUENCE
      );
      const nowProcessIdx = this.dgvOperation.findIndex(
        x => x.PROCESSSEQUENCE == nowProcessSeq
      );
        ...
        if (type === 'U' && upperProcess) {
          stepData = stepData.map(x => {
            if (x.RECIPESEQUENCE == moveTarget.RECIPESEQUENCE) {
              return {
                ...x,
                PROCESSSEQUENCE: upperProcess.PROCESSSEQUENCE,
                PROCESSID: upperProcess.PROCESSID,
              };
            }
            return x;
          });
          ...
          stepData = stepData.map(x => {
            if (x.RECIPESEQUENCE == moveTarget.RECIPESEQUENCE) {
              return {
                ...x,
                PROCESSSEQUENCE: lowerProcess.PROCESSSEQUENCE,
                PROCESSID: lowerProcess.PROCESSID,
              };
            }
            return x;
          });
      ...
      const sameTypeStep = this.dgvRecipe.filter(
        x => x.RECIPERELATIONCODE == moveTarget.RECIPERELATIONCODE
      );
      const recipetypeArr = [];
    ...
    mkTree(route, process, step) {
      ...
      for (let i = 0; i < pd.length; i++) {
        for (let j = 0; j < step.length; j++) {
          if (
            pd[i].PROCESSID == step[j].PROCESSID &&
            pd[i].PROCESSSEQUENCE == step[j].PROCESSSEQUENCE
          ) {
