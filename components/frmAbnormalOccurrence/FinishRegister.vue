<template>
    <div>
        <Dialog v-if="visibleDialog" :title="$t('MES_CommLang.MES_CommLang_00907')" width="55%" height="250"
            @close="toggleDialog">
            <v-row no-gutters style="margin-bottom: -7px;">
                <v-col cols="6" align="left">
                    <CardTitle style="margin-bottom: -10px;"> {{ $t("MES_CommLang.MES_CommLang_00907") }}</CardTitle>
                    <!-- 검교정 결과 등록 -->
                </v-col>
                                ...
                                <v-col :cols="12">
                                    <div class="form-group-wrap" style="margin-bottom: 5px;">
                                        <div class="form-group">
                                            ...
                                            <InputText ref="" :dataNm="'occurrencecode'" :disabled="false" :readonly="true"
                                                :searchOption="true" />
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <label> {{ $t("MES_CommLang.MES_CommLang_00911") }} </label> <!-- 조치판정 -->
                                            <MesSelectBox :enumID="'FinishResult'" :allYN="false" :dataNm="'FinishResult'"
                                                @comboChange="comboChange"></MesSelectBox>
                                        </div>
                                        <div class="form-group">
                                            <label> {{ $t("MES_CommLang.MES_CommLang_00912") }} </label> <!-- 조치내용 -->
                                            <InputText ref="finishComment" :dataNm="'finishComment'" :disabled="false"
                                                :readonly="false" :searchOption="true" style="width: 618px;" @input-text-set="searchInput" />
                                        </div>
        ...
        saveBtn() {
            ...
            this.saveData = this.saveData.map(x => {
                return {
                    ...
                    FINISHTIME: Utility.setFormatDate(new Date(), "YYYY-MM-DD hh:mm:ss"),
                    FINISHUSERID: this.userInfo.userId,
                    ABNORMALSTATE: "Completed",
