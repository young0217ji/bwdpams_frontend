  ...
  <div :style="{ height: '100%' }">
    <v-row :style="{ height: '100%' }">
      <v-col ref="gridArea" :cols="12" align="right">
        <!-- <v-col :cols="2"> -->
        <label
          :style="{ backgroundColor: 'green', color: 'green', float: 'left' }"
          >&nbsp;{{ $t('MES_CommLang.MES_CommLang_00446') }}</label
        >
        <!-- 색상 -->
        <label
          :style="{ fontWeight: 'bold', float: 'left', marginRight: '15px' }"
          >&nbsp;: {{ $t('MES_CommLang.MES_CommLang_00495') }}</label
        >
        <!-- 완료 -->
        <!-- </v-col>
          <v-col :cols="2"> -->
        <label
          :style="{ backgroundColor: 'blue', color: 'blue', float: 'left' }"
          >&nbsp;{{ $t('MES_CommLang.MES_CommLang_00446') }}</label
        >
        <!-- 색상 -->
        <label
          :style="{ fontWeight: 'bold', float: 'left', marginRight: '15px' }"
          >&nbsp;: {{ $t('MES_CommLang.MES_CommLang_00290') }}</label
        >
        <!-- 진행중 -->
        <!-- </v-col>
          <v-col :cols="2"> -->
        <label :style="{ backgroundColor: 'red', color: 'red', float: 'left' }"
          >&nbsp;{{ $t('MES_CommLang.MES_CommLang_00446') }}</label
        >
        <!-- 색상 -->
        <label
          :style="{ fontWeight: 'bold', float: 'left', marginRight: '15px' }"
          >&nbsp;: {{ $t('MES_CommLang.MES_CommLang_00423') }}</label
        >
        <!-- 미진행 -->
        <!-- </v-col> -->
        <!-- <v-col :cols="2"> -->
        <label
          :style="{ backgroundColor: 'yellow', color: 'yellow', float: 'left' }"
          >&nbsp;{{ $t('MES_CommLang.MES_CommLang_00446') }}</label
        >
        <!-- 색상 -->
        <label :style="{ fontWeight: 'bold', float: 'left' }"
          >&nbsp;: {{ $t('MES_CommLang.MES_CommLang_00493') }}</label
        >
        <!-- 예정 -->
        <!-- </v-col> -->
        <Button :theme-color="'primary'" :size="'small'" @click="addPMCycle">{{
          $t('MES_CommLang.MES_CommLang_00039')
        }}</Button>
        <!-- 예방보전 주기 추가 -->
      </v-col>
    ...
    <PMDetailAdd
      ref="PMDetailAdd"
      v-if="setPMDetailAddModalVisible"
      :visibleDialog="setPMDetailAddModalVisible"
      :rightClickPlanData="rightClickPlanData"
      :btnFlag="btnFlag"
      :title="'title'"
      :message="'message'"
      :callBack="() => {}"
      @visibleDialog="val => (setPMDetailAddModalVisible = val)"
      @pmDetailSaveData="pmDetailSaveData"
    />
  </div>
</template>
...
import CustomCalendarCell from '@/components/frmPMMgmtTabs/PMScheduleCustom.vue';
import PlanPop from '@/components/common/modal/PlanPop';
import ContextMenu from '@/components/frmPMMgmtTabs/ContextMenu.vue';
import PMDetail from '@/components/frmPMMgmtTabs/PMDetail';
import PMDetailAdd from '@/components/frmPMMgmtTabs/PMDetailAdd';
...
export default {
  ...
  computed: {
    ...
    gridItems() {
      return this.gridData.map(item => ({
        ...
        hldyFg:
          item.hldyFg == '1'
            ? this.$i18n.t('MES_CommLang.MES_CommLang_00343')
            : this.$i18n.t('MES_CommLang.MES_CommLang_00342'),
      })); /* 평일 */ /* 휴일 */
    },
  },
  watch: {},
  beforeCreate() {},
  async mounted() {
    this.gridDropDownList[0].dataItem[0] = this.$i18n.t(
      'MES_CommLang.MES_CommLang_00342'
    ); /* 평일 */
    this.gridDropDownList[0].dataItem[1] = this.$i18n.t(
      'MES_CommLang.MES_CommLang_00343'
    ); /* 휴일 */
    // var today = new Date();
    ...
    */
    ...
    removeClickContextMenu(clickData, btnFlag) {
      ...
      this.$refs['confirmPop'].message =
        clickData[0][0].planTitle +
        ' - ' +
        this.$i18n.t(
          'MES_MsgLang.MES_MsgLang_00243'
        ); /* 해당 예방보전일정을 삭제하시겠습니까? */
    ...
    addPMCycle(e) {
      ...
      this.visibleDialog = !this.visibleDialog;
      this.$refs['confirmPop'].title = this.$i18n.t(
        'MES_CommLang.MES_CommLang_00618'
      ); /* 이달의 계획 설정  */
      this.$refs['confirmPop'].message = this.$i18n.t(
        'MES_MsgLang.MES_MsgLang_00239'
      ); /* 현재시간 이후 모든 계획을 초기화 후 재설정합니다. 진행하시겠습니까? */
    ...
    ddChange: function (e, name, dataItem) {
      ...
      if (name == 'hldyFg') {
        e.value =
          e.value == this.$i18n.t('MES_CommLang.MES_CommLang_00343')
            ? '1'
            : '0';
      }
    ...
    async gridDataSelect(yearMonthData, workeruserid) {
      ...
      await this.mesGet({
        ...
        if (workeruserid === '' || workeruserid === undefined) {
          PMScheduleListData = res1.map((x, idx) => {
            // console.log(":x::::::",x);
            return {
              ...x,
              rowStat: 'N',
      ...
      for (var i = 0; i < res.length; i++) {
        if (
          !(
            res[i].hldyNm == null ||
            res[i].hldyNm == this.$i18n.t('MES_CommLang.MES_CommLang_00345') ||
            res[i].hldyNm == this.$i18n.t('MES_CommLang.MES_CommLang_00346')
          )
        ) {
        ...
        const res = this.mesPost({
          ...
          this.$nextTick(() => {
            if (result.returncode !== undefined && result.returncode === '0') {
              this.$emit(
                'reSearch',
                Utility.setFormatDate(saveData[0].workcomment, 'YYYYMM')
              );
              this.$refs.alertPop.show(
                'Info',
                this.$i18n.t('MES_MsgLang.MES_MsgLang_00140')
