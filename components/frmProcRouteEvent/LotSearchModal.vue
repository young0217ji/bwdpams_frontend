          ...
          <v-row>
            <v-col :sm="10" :lg="10">
              <div class="form-group-wrap">
                ...
                <div class="form-group">
                  <label style=""
                    >{{ $t('MES_CommLang.MES_CommLang_00252')
                    }}<!--재공Lot--></label
                  >
                  <Checkbox
                    ref="commonflag"
              ...
              <div class="form-group-wrap">
                <div class="form-group">
                  <label style=""
                    >{{ $t('MES_CommLang.MES_CommLang_00199')
                    }}<!--시작일자--></label
                  >
                  <DatePickerSingle
  ...
  methods: {
    searchBtn() {
      if (!this.chkLot && !this.chkDate) {
        ...
        ); /* 알림, 너무 많은 Lot이 조회되어 시스템에 문제가 발생될수 있습니다.  조회조건을 확인해 주세요. */
        return;
      }
      this.getGridData();
    },
    ...
    async getGridData() {
      ...
      const fdate =
        moment(this.fromdate).format('YYYY-MM-DD') === 'Invalid date'
