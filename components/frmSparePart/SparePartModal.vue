...
//   import multiGridHeaderMinin from "@/mixin/multiGridHeaderMinin.js";
  ...
  export default {
    ...
    // 저장
    saveBtn() {
      const dgvSparePartListData = this.gridData
      ...
      if (dgvSparePartListData.length > 0) {
        const res = this.mesPost({
          apiKey: 'mes/common/manage',
          messagename: 'TxnSparePartDefinition',
          sendParam: dgvSparePartListData,
        }).then(result => {
          this.$nextTick(() => {
    ...
    confirmDel() {
      ...
      if (delData.length > 0) {
        if (delData.filter((x) => x.rowStat !== 'C').length >= 1) {
          ...
          this.$refs['confirmPop'].message = this.gridData[0].SPAREPARTID + " - " + this.$i18n.t('MES_MsgLang.MES_MsgLang_00089'); // 선택한 데이터를 삭제 하시겠습니까?
