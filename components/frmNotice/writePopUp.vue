<template>
    <div>
        <Dialog v-if="visibleDialog" :title="$t('MES_CommLang.MES_CommLang_00916')" width="55%" height="500"
            @close="toggleDialog">
            <v-row no-gutters style="margin-bottom: -7px;">
                <v-col cols="6" align="left">
                    <CardTitle v-text="cardTitle" style="margin-bottom: -10px;"></CardTitle>
                    <!-- 글작성 -->
                </v-col>
                    ...
                    <Card style="background-color: #f1f2f7;">
                        <CardBody style="padding: 20px;">
                            <v-row no-gutters>
                                <v-col :cols="12">
                                    <div class="form-group-wrap" style="margin-bottom: 5px;">
                                        <div class="form-group">
                                            <label> {{$t("MES_CommLang.MES_CommLang_00920") }} </label> <!-- 제목 -->
                                            <InputText ref="title" :dataNm="'title'" :disabled="false"  :style="{width: '400px'}"
                                                :searchOption="true" />
                                        </div>   
                                    </div>
                                    <div class="form-group-wrap" style="margin-bottom: 5px;">
                                    <div class="form-group">
                                        <label>{{$t("MES_CommLang.MES_CommLang_00921") }}</label> <!-- 내용 -->
                                        <TextArea ref="contents" :dataNm="'contents'" :style="{ width: '800px', height: '200px' }"
                                        :auto-size="false"
                                ...
                                <div class="form-group-wrap" style="margin-bottom: 5px;">
                                <div class="form-group">
                                    <label>{{$t("MES_CommLang.MES_CommLang_00922") }}</label> <!-- 조회수 -->
                                    <InputText ref="searchCount" :disabled="false"
                                    :dataNm="'searchCount'"
                                    :readonly="true" :style="{width: '50px'}"></InputText>
                                </div>
                                </div>
                                </v-col>
                            </v-row>
...
export default {
    ...
    async mounted() {
        if(this.initData.SEARCHCOUNT == undefined || this.initData.SEARCHCOUNT == null){
        ...
        if(this.$refs['title'].textVal != null || this.$refs['title'].textVal != undefined){
            this.cardTitle = '글수정';
        }
    ...
    methods: {
        toggleDialog() {
            if(this.$refs['searchCount'].textVal != "0"){
                if(this.$refs["title"].textVal != this.initData.TITLE || this.textAreaData != this.initData.CONTENTS){
        ...
        saveBtn() {
            ...
            this.saveData = this.saveData.map(x => {
                return {
                    ...
                    LASTUPDATETIME: Utility.setFormatDate(new Date(), "YYYY-MM-DD hh:mm:ss"),
                    LASTUPDATEUSERID : this.userInfo.userId,
                }
            })        
            // 공백 Check
            if(this.saveData[0].TITLE.replace(/\s/g, '').length == 0 || this.saveData[0].CONTENTS.replace(/\s/g, '').length == 0){
                    this.$refs.alertPop.show(this.$i18n.t('MES_CommLang.MES_CommLang_00456'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00292')); /* 알림, 제목 혹은 내용은 필수 입력입니다. */
                    return;
                }

            if((this.saveData[0].TITLE == "" ||this.saveData[0].TITLE == undefined) || (this.saveData[0].CONTENTS == "" || this.saveData[0].CONTENTS == undefined)){      
                    this.$refs.alertPop.show(this.$i18n.t('MES_CommLang.MES_CommLang_00456'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00292')); /* 알림, 제목 혹은 내용은 필수 입력입니다. */
                    return;
            }
