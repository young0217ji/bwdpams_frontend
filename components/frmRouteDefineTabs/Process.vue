  ...
  methods: {
      ...
      for (let i = 0; i < saveData.length; i++) {
        if (saveData[i].WORKCENTER == '') {
          ...
          if (saveData[i].PROCESSOPERTYPE == '') {
            this.$refs.alertPop.show(
              this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
              this.$i18n.t('MES_MsgLang.MES_MsgLang_00224')
            ); /* 알림, 공정구분을 선택해주세요. */
            return;
          }
        }
    ...
    rowDdChange(e, name, dataItem) {
      ...
      if (name === 'PROCESSOPERTYPE') {
        ...
        rowData[idx].PROCESSID =
          this.RepresentativeChar +
          this.selectedWorkCenter +
          this.selectedProcessOperType;

        let selectedWorkCenterName =
          this.headerParam.gridDropDownList[0].dataItem.filter(
            x => x.ENUMVALUE == this.selectedWorkCenter
          )[0].ENUMVALUENAME;
        let selectedOperTypeName =
          this.headerParam.gridDropDownList[1].dataItem.filter(
            x => x.ENUMVALUE == this.selectedProcessOperType
          )[0].ENUMVALUENAME;

        selectedWorkCenterName = selectedWorkCenterName.substring(
          selectedWorkCenterName.indexOf('('),
          selectedWorkCenterName.lastIndexOf(')') + 1
        );
        selectedOperTypeName = selectedOperTypeName.substring(
          selectedOperTypeName.indexOf('(') + 1,
          selectedOperTypeName.lastIndexOf(')')
        );

        rowData[idx].PROCESSNAME =
          selectedOperTypeName + ' ' + selectedWorkCenterName;
      }
