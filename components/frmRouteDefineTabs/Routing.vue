  ...
  methods: {
    ...
    saveValidation() {
      const validation = this.gridData.filter(
        x => x.rowStat !== 'N' && !x.PROCESSROUTEID
      );
      if (validation.length > 0) {
        this.openAlert(
          this.$i18n.t('MES_CommLang.MES_CommLang_00414'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00046')
        ); /* 저장, 라우팅ID 값은 필수 입력값입니다. */
        return;
      }
      ...
      const dupArr = this.gridData.filter(
        x => x.rowStat === 'N' && dupChk.indexOf(x.PROCESSROUTEID) > -1
      );
      if (dupArr.length > 0) {
        this.openAlert(
          this.$i18n.t('MES_CommLang.MES_CommLang_00414'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00002')
        ); /* 저장, AREAID값이 중복 되었습니다. */
        return;
      }
      this.saveBtn();
    },
