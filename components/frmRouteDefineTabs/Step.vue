          ...
          <v-col cols="12">
            <div class="form-group-wrap form-group-style2">
              <div class="form-group">
                <Label>{{ $t('MES_CommLang.MES_CommLang_00194') }}</Label>
                <!-- 스텝ID -->
                <InputText
                  ref="recipeid"
                  :dataNm="'recipeid'"
              ...
              <div class="form-group">
                <Label>{{ $t('MES_CommLang.MES_CommLang_00195') }}</Label>
                <!-- 스텝명 -->
                <InputText
                  ref="recipename"
                  :dataNm="'recipename'"
              ...
              <div class="form-group">
                <Label>{{ $t('MES_CommLang.MES_CommLang_00197') }}</Label>
                <!-- 스텝정의구분 -->
                <MesSelectBox
  ...
  methods: {
    ...
    saveValidation() {
      ...
      const validation = this.gridData.filter(
        x => x.rowStat !== 'N' && !x.RECIPEID
      );
      if (validation.length > 0) {
        this.openAlert(
          this.$i18n.t('MES_CommLang.MES_CommLang_00414'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00097')
        ); /* 저장, 스텝ID 값은 필수 입력값입니다. */
        return;
      }
      const validation2 = this.gridData.filter(
        x => x.rowStat !== 'N' && !x.RECIPENAME
      );
      if (validation2.length > 0) {
        this.openAlert(
          this.$i18n.t('MES_CommLang.MES_CommLang_00414'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00099')
        ); /* 저장, 스텝명 값은 필수 입력값입니다. */
        return;
      }
      const dupChk = [];
      ...
      const dupArr = this.gridData.filter(
        x => x.rowStat === 'N' && dupChk.indexOf(x.RECIPEID) > -1
      );
      if (dupArr.length > 0) {
        this.openAlert(
          this.$i18n.t('MES_CommLang.MES_CommLang_00414'),
          this.$i18n.t('MES_MsgLang.MES_MsgLang_00098')
        ); /* 저장, 스텝ID 값이 중복 되었습니다. */
        return;
      }
      this.saveBtn();
