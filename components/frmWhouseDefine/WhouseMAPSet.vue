  ...
  methods: {
    ...
    onItemClick(event) {
      ...
      this.selectedDisplay =
        '[' + this.treeActiveItem.AREAID + '] ' + this.treeActiveItem.AREANAME;
      ...
      if (this.treeActiveItem.depth !== 0) {
        if (!this.isNullOrEmpty(this.treeActiveItem.WAREHOUSENAME)) {
          this.$refs.alertPop.show(
            '알림',
            '해당 Area (' +
              this.treeActiveItem.AREANAME +
              ') 는 아래 WareHouse로 설정되어 있습니다.<br/>확인해 주시기 바랍니다.<br/><br/>Warehouse : ' +
              this.treeActiveItem.WAREHOUSENAME
          );
          return;
        } else {
    ...
    delBeforChk() {
      if (this.gridData.filter(x => x.selected).length > 0) {
        ...
        this.$refs['confirmPop'].message =
          '선택된 데이터(' + selectedTitle + ')를 삭제하시겠습니까?';
