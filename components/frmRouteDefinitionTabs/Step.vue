          ...
          <v-col cols="12">
            <div class="form-group-wrap form-group-style2">
              <div class="form-group">
                ...
                <InputText ref="recipeid" :dataNm="'recipeid'" :disabled="false" @input-text-set="searchInput"
                  :searchOption="true" :searchFunc="searchBtn" />
              </div>
              <div class="form-group">
                <Label>{{ $t("MES_CommLang.MES_CommLang_00195") }}</Label> <!-- 스텝명 -->
                <InputText ref="recipename" :dataNm="'recipename'" :disabled="false" @input-text-set="searchInput"
                  :searchOption="true" :searchFunc="searchBtn" />
              </div>
              <div class="form-group">
                <Label>{{ $t("MES_CommLang.MES_CommLang_00197") }}</Label> <!-- 스텝정의구분 -->
                <MesSelectBox :enumID="'RecipeDefType'" :allYN="true" :dataNm="'recipedeftype'"
          ...
          <v-col cols="12" :style="{ height: '100%' }">
            <div ref="divWrapper" :style="{ height: '100%' }">
              <KendoGrid ref="rowGrid" :gridHeight="gridHeight" :gridItems="gridData" :sortable="true" :gridPageData="gridPageData"
                ...
                :gridDropDownList="gridDropDownList" :isPaging="true" @selectionchange="onSelectionchange"
...
export default {
  ...
  data() {
    return {
      ...
      gridOriData: [],
      gridDropDownList: [
        { dataVal: 'RECIPEDEFTYPE', dataItem: [] },
  ...
  methods: {
    ...
    saveValidation() {
      ...
      const validation = this.gridData.filter(x => x.rowStat !== "N" && !x.RECIPEID)
      if (validation.length > 0) {
        this.openAlert(this.$i18n.t('MES_CommLang.MES_CommLang_00414'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00097')) /* 저장, 스텝ID 값은 필수 입력값입니다. */
        return;
      }
      const validation2 = this.gridData.filter(x => x.rowStat !== "N" && !x.RECIPENAME)
      if (validation2.length > 0) {
        this.openAlert(this.$i18n.t('MES_CommLang.MES_CommLang_00414'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00099')) /* 저장, 스텝명 값은 필수 입력값입니다. */
        return;
      }
      const dupChk = [];
      this.gridData.forEach(x => {
        if (x.rowStat !== "N") {
          dupChk.push(x.RECIPEID)
        }
      })
      const dupArr = this.gridData.filter(x => x.rowStat === "N" && dupChk.indexOf(x.RECIPEID) > -1)
      if (dupArr.length > 0) {
        this.openAlert(this.$i18n.t('MES_CommLang.MES_CommLang_00414'), this.$i18n.t('MES_MsgLang.MES_MsgLang_00098')) /* 저장, 스텝ID 값이 중복 되었습니다. */
        return;
      }
      this.saveBtn();
    },
    ...
    delConfirm() {
      ...
      if (chk.length == 0 && this.newRowChk.length == 0) { // 체크한 행 없을 때
        this.$refs.alertPop.title = this.$i18n.t('MES_CommLang.MES_CommLang_00412'); /* 저장 */
