<template>
  <div>
    ...
    <v-row no-gutters>
      ...
      <v-col :sm="3" :lg="3" align="right" v-if="screenType === 'frmMaterialReceive'">
        <Button :theme-color="'primary'" :size="'medium'" @click="receiveDetailCancelBtn()">입고취소</Button>
      </v-col>
    </v-row>
...
export default { 
  ...
  methods: {
    ...
    deleteData(){
      const deleteData = this.gridData.filter(x=>x.selected)
      .map(x => {
        return {
            ...x,
