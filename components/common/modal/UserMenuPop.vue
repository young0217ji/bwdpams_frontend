<template>
      ...
      >
            ...
            <v-col :cols="5" >
                ...
                <v-row>
                    <v-col :cols="12" align="center" >
                        <Button :theme-color="'secondary'" :style="{ width: '48%', height:'100%'}" @click="upBtn" >▲</Button> <!-- 저장 -->
                        <Button :theme-color="'secondary'" :style="{ width: '48%', height:'100%'}" @click="downBtn">▼</Button> <!-- 닫기 -->
                    </v-col>
                </v-row>
            </v-col>
        </v-row>
        <v-row>
            ...
            <v-col :cols="12" align="right">
                <Button :theme-color="'primary'" @click="saveBtn" >{{ $t("MES_CommLang.MES_CommLang_00414") }}</Button> <!-- 저장 -->
  ...
  export default { 
    ...
    methods: {
        ...
        // 등록 버튼
        addBtn(){
            const idxList = this.gridData.map(x=> {return x.idx});
            const nextIdx = this.gridData.length>0? Math.max.apply(null,idxList)+1:1;
            if(!this.isNullOrEmpty(this.treeActiveItem.URL) && this.gridData.filter(x => x.menuId==this.treeActiveItem.MENUID).length<1){
                this.gridData.push({
            ...
            }else if(this.gridData.filter(x => x.menuId==this.treeActiveItem.MENUID).length>0){
        ...
        downBtn(){
            ...
            if(selectedIdx==this.gridData.length || this.isNullOrEmpty(selectedIdx)){
                return;
            }
            const selectRow = JSON.parse(JSON.stringify(this.gridData[selectedIdx]));
            this.gridData.splice(selectedIdx,1);
            ...
            saveData.map(x => {
                this.gridOriData.map(y => {
                    if(x.menuId == y.menuId && x.menuSequence != y.menuSequence){
                        sendParam.push({...x, plantId: plantId, userId: userId, flag:'U'});
                    }
                })
