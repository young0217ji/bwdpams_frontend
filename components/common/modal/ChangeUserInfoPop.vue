<template>
  <Dialog v-if="visibleDialog" :title="$i18n.t('MES_CommLang.MES_CommLang_00628')" @close="toggleDialog" height="380px"
    width="30%">

    <v-row>
      <v-col :cols="12">
        <Input v-model.trim="password" type="password" :label="$i18n.t('MES_MsgLang.MES_MsgLang_00215')"
    ...
    <v-row>
      <v-col :cols="12">
        <Input v-model.trim="name" type="string" :style="{ width: '100%' }" :label="$i18n.t('UserRoleMngPage.label1')"
          height="44"></Input> <!-- 사용자명 -->
      </v-col>
    </v-row>
    <v-row>
      <v-col :cols="12">
        <Input v-model.trim="email" type="string" :label="$i18n.t('UserRoleMngPage.label6')" :style="{ width: '100%' }"
          height="44"></Input>
        <!-- 이메일 -->
      </v-col>
    </v-row>
    <v-row>
      <v-col :cols="12">
        <Input v-model.trim="phoneNumber" type="string" :label="$i18n.t('UserRoleMngPage.label7')"
          :style="{ width: '100%' }" height="44"></Input>
        <!-- 전화번호 -->
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    initial() {
      const { name: uname, email: uemail, phoneNumber: uphoneNumber } = this.$auth.$state.user;
      this.name = uname;
      this.email = uemail;
