<template>
  <div>
                ...
                <div class="area-form">
                  <div class="area-th">
                    <label style="float: left; margin-top: 5px">{{
                      $t('MES_CommLang.MES_CommLang_00622')
                    }}</label
                    ><!-- 예비품 코드 -->
                  </div>
                  <div class="area-td">
                  ...
                  <div class="area-th">
                    <label
                      ...
                      >{{ $t('MES_CommLang.MES_CommLang_00623') }}</label
                    >
                    <!-- 예비품명 -->
                  </div>
                  <div class="area-td">
            ...
            <div class="detail-area">
              <div class="left-area">
                <div class="area-form">
                  <div class="area-th">
                    <label
                      style="float: left; margin-top: 5px; margin-right: 14px"
                      >{{ $t('MES_CommLang.MES_CommLang_00294') }}</label
                    >
                    <!-- 창고코드-->
                  </div>
                  <div class="area-td">
                    ...
                    <Button
                      ...
                      @click="WarehousePlusBtnClick"
                    ></Button>
                  </div>
                  <div class="area-th">
                    <label
                      style="float: left; margin-top: 5px; margin-right: 5px"
                      >{{ $t('MES_CommLang.MES_CommLang_00293') }}</label
                    >
                    <!--창고명-->
                  </div>
                  <div class="area-td">
    ...
    />
    <WarehouseCodeModal
      ref="WarehouseCodeModal"
      v-if="warehouseCodeModalVisible"
      :visibleDialog="warehouseCodeModalVisible"
      :title="warehouseCodeModalTitle"
      :selected-field="selectedField"
      @visibleDialog="val => (warehouseCodeModalVisible = val)"
      @warehouseSelectedRow="warehouseSelectedRow"
    />
...
import WarehouseCodeModal from '@/components/frmMatMgmt/WarehouseCodeModal.vue';
...
export default {
  ...
  async mounted() {
    ...
    this.warehouseCodeModalTitle = this.$i18n.t(
      'MES_CommLang.MES_CommLang_00627'
    ); // 창고 선택/등록
    this.stringNum =
      this.workType == 'input'
        ? this.$i18n.t('MES_CommLang.MES_CommLang_00218')
        : this.$i18n.t(
            'MES_CommLang.MES_CommLang_00303'
          ); /* 입고수량, 출고수량 */
  ...
  methods: {
    ...
    save() {
      if (this.workType == 'input') {
        ...
        if (
          saveData[0].MATERIALID == '' ||
          saveData[0].MATERIALID == 'undefined'
        ) {
          if (
            saveData[0].WAREHOUSEID == '' ||
            saveData[0].WAREHOUSEID == 'undefined'
          ) {
            this.$refs.alertPop.show(
              this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
              this.$i18n.t('Mes_MsgLang.MES_MsgLang_00245')
            ); /*알림, 예비품코드와 창고코드를 입력해주세요 */
            return;
          }
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('Mes_MsgLang.MES_MsgLang_00246')
          ); /*알림, 예비품코드를 입력해주세요 */
          return;
        } else if (
          saveData[0].WAREHOUSEID == '' ||
          ...
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('Mes_MsgLang.MES_MsgLang_00247')
          ); /*알림, 창고코드를 입력해주세요 */
          return;
        }
        if (saveData[0].INQTY == '') {
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('Mes_MsgLang.MES_MsgLang_00248')
          ); /*알림, 입고수량를 입력해주세요 */
          return;
        }
        ...
        if (saveData[0].OUTQTY == '') {
          this.$refs.alertPop.show(
            this.$i18n.t('MES_CommLang.MES_CommLang_00456'),
            this.$i18n.t('Mes_MsgLang.MES_MsgLang_00249')
          ); /*알림, 출고수량를 입력해주세요 */
          return;
        }
