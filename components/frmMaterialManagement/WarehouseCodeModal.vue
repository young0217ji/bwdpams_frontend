<template>
  <div>
    ...
    <Dialog v-if="visibleDialog" :title="title" width="70%" height="650" @close="toggleDialog">
      <v-row no-gutters style="margin-bottom: 20px;">
        <v-col :sm="7" :lg="7">
          <div class="form-group-wrap">
            <div class="form-group">
              <label>{{ $t("MES_CommLang.MES_CommLang_00294") }}</label><!--창고코드-->
              <InputText ref="warehouseCode" :dataNm="'warehouseCode'" :disabled="false" :style="{ width: '160px' }"
                :searchOption="true" :searchFunc="searchBtn" @input-text-set="searchInput" />
            </div>
            <div class="form-group">
              <label>{{ $t("MES_CommLang.MES_CommLang_00293") }}</label><!--창고명-->
              <InputText ref="warehouseName" :dataNm="'warehouseName'" :disabled="false" :style="{ width: '160px' }"
                :searchOption="true" :searchFunc="searchBtn" @input-text-set="searchInput" />
            </div>
          </div>
        </v-col>
        <v-col :sm="5" :lg="5" align="right">
          ...
          <kbutton :theme-color="'primary'" :size="'medium'" :icon="'save'" @click="selectRow">{{
      ...
      <div ref="divWrapper" style="height: calc(100% - 94px)">
        <KendoGrid ref="rowGrid" :gridHeight="gridHeight" :gridItems="gridData" :sortable="true"
          :columns="headerParam.gridHeader" :gridDropDownList="headerParam.enumComboBox" :resizable="true"
          :edit-field="'inEdit'" :readonly="false" :selected-field="selectedField" :isPaging="true"
          :reSetPage="this.reSetPage" @gridrowclick="onRowClick" @rowdblclick="rowdblclick" />
      </div>
    </Dialog>
  ...
  methods: {
    ...
    rowdblclick(event) {
      if (event.dataItem.rowStat != 'N') {
        this.$refs.alertPop.show(this.$i18n.t('MES_CommLang.MES_CommLang_00456'), '저장 후 선택해주세요.'); /* 알림, 저장 후 선택해주세요. */
        return;
      }
